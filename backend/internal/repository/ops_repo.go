...
const insertOpsRequestCaptureSQL = `
INSERT INTO ops_request_captures (
  outcome,
  request_id,
  client_request_id,
  user_id,
  ...
  inbound_request_body_truncated,
...
func (r *opsRepository) InsertRequestCapture(ctx context.Context, input *service.OpsInsertRequestCaptureInput) (int64, error) {
	if r == nil || r.db == nil {
		return 0, fmt.Errorf("nil ops repository")
	}
	if input == nil {
	...
	err := r.db.QueryRowContext(
		ctx,
		insertOpsRequestCaptureSQL+" RETURNING id",
		opsInsertRequestCaptureArgs(input)...,
	).Scan(&id)
	if err != nil {
		return 0, err
	}
...
func opsInsertRequestCaptureArgs(input *service.OpsInsertRequestCaptureInput) []any {
	return []any{
		opsNullString(input.Outcome),
		opsNullString(input.RequestID),
		opsNullString(input.ClientRequestID),
		opsNullInt64(input.UserID),
		opsNullInt64(input.APIKeyID),
		...
		opsNullString(input.RequestPayloadHash),
		opsNullString(input.UserAgent),
		opsNullInt(input.StatusCode),
		opsNullInt(input.UpstreamStatusCode),
		opsNullString(input.ErrorMessage),
		opsNullString(input.InboundRequestHeadersJSON),
		opsNullString(input.InboundRequestBody),
		input.InboundRequestTruncated,
		opsNullInt(input.InboundRequestBytes),
		opsNullString(input.UpstreamRequestMethod),
		opsNullString(input.UpstreamRequestURL),
		opsNullString(input.UpstreamRequestHeadersJSON),
		opsNullString(input.UpstreamRequestBody),
		input.UpstreamRequestTruncated,
		opsNullInt(input.UpstreamRequestBytes),
		opsNullString(input.UpstreamResponseHeadersJSON),
		opsNullString(input.UpstreamResponseBody),
		input.UpstreamResponseTruncated,
		opsNullInt(input.UpstreamResponseBytes),
		opsNullInt(input.InputTokens),
		opsNullInt(input.OutputTokens),
		opsNullInt(input.CacheCreationTokens),
		opsNullInt(input.CacheReadTokens),
