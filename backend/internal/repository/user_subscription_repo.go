...
func (r *userSubscriptionRepository) EnsureFiveHourWindow(ctx context.Context, id int64, newWindowStart time.Time) error {
	const updateSQL = `
		UPDATE user_subscriptions
		SET
			five_hour_usage_usd = 0,
			five_hour_window_start = $1,
			updated_at = NOW()
		WHERE id = $2
	...
	client := clientFromContext(ctx, r.client)
	_, err := client.ExecContext(ctx, updateSQL, newWindowStart, id)
	if err != nil {
		return err
	}
	return nil
		...
		SET
			five_hour_usage_usd = CASE
				WHEN us.five_hour_window_start IS NULL
					OR us.five_hour_window_start <= NOW() - INTERVAL '5 hours'
				THEN $1
