...
func (s *httpUpstreamService) Do(req *http.Request, proxyURL string, accountID int64, accountConcurrency int) (*http.Response, error) {
	outboundTrace, req := httptrace.StartOutbound(httptrace.Default(), req)
	...
	if err := decodeUpstreamResponse(resp); err != nil {
		_ = resp.Body.Close()
		atomic.AddInt64(&entry.inFlight, -1)
		atomic.StoreInt64(&entry.lastUsed, time.Now().UnixNano())
		return nil, err
	...
	resp = outboundTrace.WrapResponse(resp)
...
func (s *httpUpstreamService) DoWithTLS(req *http.Request, proxyURL string, accountID int64, accountConcurrency int, enableTLSFingerprint bool) (*http.Response, error) {
	...
	if err := decodeUpstreamResponse(resp); err != nil {
		...
		atomic.AddInt64(&entry.inFlight, -1)
		atomic.StoreInt64(&entry.lastUsed, time.Now().UnixNano())
		slog.Debug("tls_fingerprint_decode_response_failed", "account_id", accountID, "error", err)
		return nil, err
	}
...
func buildUpstreamTransportWithTLSFingerprint(settings poolSettings, proxyURL *url.URL, profile *tlsfingerprint.Profile) (*http.Transport, error) {
	...
	rootCAs, _ := extracerts.RootCAs()

	// 根据代理类型选择合适的 TLS 指纹 Dialer
	if proxyURL == nil {
		// 直连：使用 TLSFingerprintDialer
		slog.Debug("tls_fingerprint_transport_direct")
		dialer := tlsfingerprint.NewDialer(profile, nil).WithRootCAs(rootCAs)
		...
		case "socks5", "socks5h":
			...
			socks5Dialer := tlsfingerprint.NewSOCKS5ProxyDialer(profile, proxyURL).WithRootCAs(rootCAs)
			transport.DialTLSContext = socks5Dialer.DialTLSContext
		case "http", "https":
			// HTTP/HTTPS 代理：使用 HTTPProxyDialer（CONNECT 隧道）
			slog.Debug("tls_fingerprint_transport_http_connect", "proxy", proxyURL.Host)
			httpDialer := tlsfingerprint.NewHTTPProxyDialer(profile, proxyURL).WithRootCAs(rootCAs)
...
func decodeUpstreamResponse(resp *http.Response) error {
	if resp == nil || resp.Body == nil {
		return nil
	}
	encodingHeader := strings.TrimSpace(resp.Header.Get("Content-Encoding"))
	if encodingHeader == "" {
		return nil
	}
	...
	for i := len(encodings) - 1; i >= 0; i-- {
		...
		case "gzip", "x-gzip":
			...
			if err != nil {
				return fmt.Errorf("decode upstream gzip response: %w", err)
			}
			reader = gzReader
			closers = append([]io.Closer{gzReader}, closers...)
		case "deflate":
			flateReader := flate.NewReader(reader)
			reader = flateReader
			closers = append([]io.Closer{flateReader}, closers...)
		case "br":
			reader = brotli.NewReader(reader)
		case "zstd":
			zstdReader, err := zstd.NewReader(reader)
			if err != nil {
				return fmt.Errorf("decode upstream zstd response: %w", err)
			}
			reader = zstdReader
			closers = append([]io.Closer{noErrCloser{closeFn: zstdReader.Close}}, closers...)
		default:
			return fmt.Errorf("unsupported upstream content-encoding: %s", encoding)
		}
	}

	resp.Body = &chainedReadCloser{reader: reader, closers: closers}
