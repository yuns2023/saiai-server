...
func (r *usageLogRepository) ListWithFilters(ctx context.Context, params pagination.PaginationParams, filters UsageLogFilters) ([]service.UsageLog, *pagination.PaginationResult, error) {
	...
	conditions, args = appendSessionIDWhereCondition(conditions, args, "session_id", filters.SessionID)
	...
	conditions, args = appendAccountSwitchWhereCondition(conditions, args, "usage_logs", filters)
	...
	if err := r.hydrateUsageLogSessionAccounts(ctx, logs, filters); err != nil {
		return nil, nil, err
	}
	if err := r.hydrateUsageLogFailoverEvents(ctx, logs); err != nil {
		return nil, nil, err
	}
...
func shouldUseFastUsageLogTotal(filters UsageLogFilters) bool {
	...
	return filters.UserID == 0 && filters.APIKeyID == 0 && filters.AccountID == 0 && filters.SessionID == ""
}
...
func (r *usageLogRepository) GetUsageTrendWithFilters(ctx context.Context, startTime, endTime time.Time, granularity string, userID, apiKeyID, accountID, groupID int64, model string, requestType *int16, stream *bool, billingType *int8, sessionID string) (results []TrendDataPoint, err error) {
	if shouldUsePreaggregatedTrend(granularity, userID, apiKeyID, accountID, groupID, model, requestType, stream, billingType, sessionID) {
	...
	query, args = appendSessionIDQueryFilter(query, args, "session_id", sessionID)
...
func shouldUsePreaggregatedTrend(granularity string, userID, apiKeyID, accountID, groupID int64, model string, requestType *int16, stream *bool, billingType *int8, sessionID string) bool {
...
func (r *usageLogRepository) GetModelStatsWithFilters(ctx context.Context, startTime, endTime time.Time, userID, apiKeyID, accountID, groupID int64, requestType *int16, stream *bool, billingType *int8, sessionID string) (results []ModelStat, err error) {
	return r.getModelStatsWithFiltersBySource(ctx, startTime, endTime, userID, apiKeyID, accountID, groupID, requestType, stream, billingType, usagestats.ModelSourceRequested, sessionID)
}
...
func (r *usageLogRepository) GetModelStatsWithFiltersBySource(ctx context.Context, startTime, endTime time.Time, userID, apiKeyID, accountID, groupID int64, requestType *int16, stream *bool, billingType *int8, source string, sessionID string) (results []ModelStat, err error) {
	return r.getModelStatsWithFiltersBySource(ctx, startTime, endTime, userID, apiKeyID, accountID, groupID, requestType, stream, billingType, source, sessionID)
}
...
func (r *usageLogRepository) GetGroupStatsWithFilters(ctx context.Context, startTime, endTime time.Time, userID, apiKeyID, accountID, groupID int64, requestType *int16, stream *bool, billingType *int8, sessionID string) (results []usagestats.GroupStat, err error) {
	query := `
		SELECT
	...
	query, args = appendSessionIDQueryFilter(query, args, "ul.session_id", sessionID)
	query, args = appendRequestTypeOrStreamQueryFilter(query, args, requestType, stream)
	if billingType != nil {
...
func (r *usageLogRepository) GetStatsWithFilters(ctx context.Context, filters UsageLogFilters) (*UsageStats, error) {
	...
	endpoints, endpointErr := r.GetEndpointStatsWithFilters(ctx, start, end, filters.UserID, filters.APIKeyID, filters.AccountID, filters.GroupID, filters.Model, filters.RequestType, filters.Stream, filters.BillingType, filters.SessionID)
	if endpointErr != nil {
		logger.LegacyPrintf("repository.usage_log", "GetEndpointStatsWithFilters failed in GetStatsWithFilters: %v", endpointErr)
		endpoints = []EndpointStat{}
	}
	upstreamEndpoints, upstreamEndpointErr := r.GetUpstreamEndpointStatsWithFilters(ctx, start, end, filters.UserID, filters.APIKeyID, filters.AccountID, filters.GroupID, filters.Model, filters.RequestType, filters.Stream, filters.BillingType, filters.SessionID)
	...
	endpointPaths, endpointPathErr := r.getEndpointPathStatsWithFilters(ctx, start, end, filters.UserID, filters.APIKeyID, filters.AccountID, filters.GroupID, filters.Model, filters.RequestType, filters.Stream, filters.BillingType, filters.SessionID)
...
func (r *usageLogRepository) getEndpointStatsByColumnWithFilters(ctx context.Context, endpointColumn string, startTime, endTime time.Time, userID, apiKeyID, accountID, groupID int64, model string, requestType *int16, stream *bool, billingType *int8, sessionID string) (results []EndpointStat, err error) {
	actualCostExpr := "COALESCE(SUM(actual_cost), 0) as actual_cost"
	if accountID > 0 && userID == 0 && apiKeyID == 0 {
		actualCostExpr = "COALESCE(SUM(total_cost * COALESCE(account_rate_multiplier, 1)), 0) as actual_cost"
	}
...
func (r *usageLogRepository) getEndpointPathStatsWithFilters(ctx context.Context, startTime, endTime time.Time, userID, apiKeyID, accountID, groupID int64, model string, requestType *int16, stream *bool, billingType *int8, sessionID string) (results []EndpointStat, err error) {
	actualCostExpr := "COALESCE(SUM(actual_cost), 0) as actual_cost"
	if accountID > 0 && userID == 0 && apiKeyID == 0 {
		actualCostExpr = "COALESCE(SUM(total_cost * COALESCE(account_rate_multiplier, 1)), 0) as actual_cost"
	}
...
func (r *usageLogRepository) GetEndpointStatsWithFilters(ctx context.Context, startTime, endTime time.Time, userID, apiKeyID, accountID, groupID int64, model string, requestType *int16, stream *bool, billingType *int8, sessionID string) ([]EndpointStat, error) {
	return r.getEndpointStatsByColumnWithFilters(ctx, "inbound_endpoint", startTime, endTime, userID, apiKeyID, accountID, groupID, model, requestType, stream, billingType, sessionID)
}

// GetUpstreamEndpointStatsWithFilters returns upstream endpoint statistics with optional filters.
func (r *usageLogRepository) GetUpstreamEndpointStatsWithFilters(ctx context.Context, startTime, endTime time.Time, userID, apiKeyID, accountID, groupID int64, model string, requestType *int16, stream *bool, billingType *int8, sessionID string) ([]EndpointStat, error) {
	return r.getEndpointStatsByColumnWithFilters(ctx, "upstream_endpoint", startTime, endTime, userID, apiKeyID, accountID, groupID, model, requestType, stream, billingType, sessionID)
}
...
func (r *usageLogRepository) listUsageLogsWithPagination(ctx context.Context, whereClause string, args []any, params pagination.PaginationParams) ([]service.UsageLog, *pagination.PaginationResult, error) {
	...
	logs, err := r.queryUsageLogsWithEffectiveReasoningEffort(ctx, query, listArgs...)
	if err != nil {
		return nil, nil, err
	}
...
func (r *usageLogRepository) queryUsageLogsWithEffectiveReasoningEffort(ctx context.Context, query string, args ...any) ([]service.UsageLog, error) {
	logs, err := r.queryUsageLogs(ctx, query, args...)
	if err != nil {
		return nil, err
	}
	if err := r.hydrateUsageLogEffectiveReasoningEffort(ctx, logs); err != nil {
		return nil, err
	}
	return logs, nil
}

type usageLogReasoningEffortTarget struct {
	index     int
	apiKeyID  int64
	sessionID string
	model     string
...
func (r *usageLogRepository) hydrateUsageLogEffectiveReasoningEffort(ctx context.Context, logs []service.UsageLog) error {
	if len(logs) == 0 {
		return nil
	}

	targets := make([]usageLogReasoningEffortTarget, 0)
	for i := range logs {
		if logs[i].ReasoningEffort != nil && strings.TrimSpace(*logs[i].ReasoningEffort) != "" {
			logs[i].ReasoningEffortEffective = logs[i].ReasoningEffort
			continue
		}
		if !shouldInheritUsageLogReasoningEffort(logs[i]) {
			continue
		}
		targets = append(targets, usageLogReasoningEffortTarget{
			index:     i,
			apiKeyID:  logs[i].APIKeyID,
			sessionID: strings.TrimSpace(*logs[i].SessionID),
	...
	args := make([]any, 0, len(targets)*5+3)
	for i, target := range targets {
		if i > 0 {
			_, _ = values.WriteString(", ")
		}
		base := len(args) + 1
		_, _ = values.WriteString(fmt.Sprintf("($%d::int, $%d::bigint, $%d::text, $%d::text, $%d::timestamptz)", base, base+1, base+2, base+3, base+4))
		args = append(args, target.index, target.apiKeyID, target.sessionID, target.model, target.createdAt)
	}

	streamTypeArg := len(args) + 1
	unknownTypeArg := len(args) + 2
	windowSecondsArg := len(args) + 3
	args = append(args,
		int16(service.RequestTypeStream),
		int16(service.RequestTypeUnknown),
		int64(usageLogReasoningEffortInheritanceWindow/time.Second),
	)

	query := fmt.Sprintf(`
WITH target(input_idx, api_key_id, session_id, model, created_at) AS (
  VALUES %s
)
SELECT target.input_idx, ref.reasoning_effort
FROM target
JOIN LATERAL (
  SELECT u.reasoning_effort
  FROM usage_logs u
  WHERE u.api_key_id = target.api_key_id
    AND u.session_id = target.session_id
    AND u.model = target.model
    AND u.inbound_endpoint = '/v1/messages'
    AND u.reasoning_effort IS NOT NULL
    AND btrim(u.reasoning_effort) <> ''
    AND (
      u.request_type = $%d
      OR (u.request_type = $%d AND u.stream = TRUE AND u.openai_ws_mode = FALSE)
    )
    AND u.created_at BETWEEN target.created_at - ($%d::bigint * INTERVAL '1 second')
                         AND target.created_at + ($%d::bigint * INTERVAL '1 second')
  ORDER BY ABS(EXTRACT(EPOCH FROM (u.created_at - target.created_at))), u.id DESC
  LIMIT 1
) ref ON TRUE
`, values.String(), streamTypeArg, unknownTypeArg, windowSecondsArg, windowSecondsArg)
	...
	for rows.Next() {
		...
		if err := rows.Scan(&inputIdx, &effort); err != nil {
			return err
		}
		if inputIdx < 0 || inputIdx >= len(logs) || strings.TrimSpace(effort) == "" {
			continue
		}
		value := effort
		logs[inputIdx].ReasoningEffortEffective = &value
		logs[inputIdx].ReasoningEffortInherited = true
	}
	return rows.Err()
}

func shouldInheritUsageLogReasoningEffort(log service.UsageLog) bool {
	if log.APIKeyID <= 0 || log.SessionID == nil || strings.TrimSpace(*log.SessionID) == "" {
		return false
	}
	if strings.TrimSpace(log.Model) == "" || log.CreatedAt.IsZero() {
		return false
	}
	if log.InboundEndpoint == nil || strings.TrimSpace(*log.InboundEndpoint) != "/v1/messages" {
		return false
	}
	return log.EffectiveRequestType() == service.RequestTypeSync
}
...
type usageLogSessionKey struct {
	apiKeyID  int64
	groupID   int64
	sessionID string
}

func usageLogSessionKeyFromLog(log service.UsageLog) (usageLogSessionKey, bool) {
	if log.SessionID == nil {
		return usageLogSessionKey{}, false
	}
	...
	return usageLogSessionKey{apiKeyID: log.APIKeyID, groupID: groupID, sessionID: sessionID}, true
}

func (r *usageLogRepository) hydrateUsageLogSessionAccounts(ctx context.Context, logs []service.UsageLog, filters UsageLogFilters) error {
	if len(logs) == 0 {
		return nil
	}

	sessionSet := make(map[string]struct{})
	apiKeySet := make(map[int64]struct{})
	hasSession := false
	for i := range logs {
		key, ok := usageLogSessionKeyFromLog(logs[i])
		if !ok {
			continue
		}
		hasSession = true
		sessionSet[key.sessionID] = struct{}{}
		apiKeySet[key.apiKeyID] = struct{}{}
	}
	if !hasSession {
		return nil
	}
	...
	apiKeyIDs := setToSlice(apiKeySet)

	var startArg any
	var endArg any
	if filters.StartTime != nil {
		startArg = *filters.StartTime
	}
	if filters.EndTime != nil {
		endArg = *filters.EndTime
	}

	rows, err := r.sql.QueryContext(ctx, `
SELECT
  ul.api_key_id,
  COALESCE(ul.group_id, 0) AS group_key,
  ul.session_id,
  ul.account_id,
  COALESCE(a.name, '') AS account_name
FROM usage_logs ul
LEFT JOIN accounts a ON a.id = ul.account_id
WHERE ul.session_id = ANY($1)
  AND ul.api_key_id = ANY($2)
  AND ($3::timestamptz IS NULL OR ul.created_at >= $3)
  AND ($4::timestamptz IS NULL OR ul.created_at < $4)
GROUP BY ul.api_key_id, COALESCE(ul.group_id, 0), ul.session_id, ul.account_id, a.name
ORDER BY MIN(ul.created_at), ul.account_id
`, pq.Array(sessionIDs), pq.Array(apiKeyIDs), startArg, endArg)
	if err != nil {
		return err
	}
	defer func() { _ = rows.Close() }()
	...
	type accountContext struct {
		accounts []service.UsageLogSessionAccount
	}
	contexts := make(map[usageLogSessionKey]*accountContext)
	for rows.Next() {
		var (
			apiKeyID    int64
			groupID     int64
		...
		if err := rows.Scan(&apiKeyID, &groupID, &sessionID, &accountID, &accountName); err != nil {
			return err
		}
		key := usageLogSessionKey{apiKeyID: apiKeyID, groupID: groupID, sessionID: sessionID}
		ctx := contexts[key]
		if ctx == nil {
			ctx = &accountContext{}
			contexts[key] = ctx
		}
		ctx.accounts = append(ctx.accounts, service.UsageLogSessionAccount{
			ID:   accountID,
			Name: accountName,
		})
	}
	...
	for i := range logs {
		...
		logs[i].SessionAccountCount = len(ctx.accounts)
		logs[i].SessionAccounts = append([]service.UsageLogSessionAccount(nil), ctx.accounts...)
	}
	return nil
}

func (r *usageLogRepository) hydrateUsageLogFailoverEvents(ctx context.Context, logs []service.UsageLog) error {
	if len(logs) == 0 {
		return nil
	}
	...
	for i := range logs {
		requestID := strings.TrimSpace(logs[i].RequestID)
		if requestID == "" {
			continue
		}
		if _, exists := requestIndex[requestID]; !exists {
			requestIDs = append(requestIDs, requestID)
		}
		requestIndex[requestID] = append(requestIndex[requestID], i)
	}
	if len(requestIDs) == 0 {
		return nil
	}
...
SELECT
  oe.request_id,
  COALESCE(oe.account_id, 0) AS source_account_id,
  COALESCE(a.name, '') AS source_account_name,
  COALESCE(oe.upstream_status_code, 0) AS upstream_status_code,
  COALESCE(oe.error_type, '') AS error_type,
  COALESCE(oe.error_source, '') AS error_source,
  LEFT(COALESCE(oe.error_message, ''), 240) AS error_message,
  LEFT(COALESCE(NULLIF(oe.upstream_error_detail, ''), NULLIF(oe.upstream_error_message, ''), ''), 1000) AS upstream_detail,
  oe.created_at
FROM ops_error_logs oe
LEFT JOIN accounts a ON a.id = oe.account_id
WHERE oe.request_id = ANY($1)
  AND oe.status_code >= 200
  AND oe.status_code < 300
  AND oe.error_phase = 'upstream'
  AND oe.error_type = 'upstream_error'
  AND oe.error_message LIKE 'Recovered upstream error%'
...
`, pq.Array(requestIDs))
	...
	for rows.Next() {
		...
		event := service.UsageLogFailoverEvent{
		...
		for _, idx := range requestIndex[requestID] {
			logs[idx].FailoverEvents = append(logs[idx].FailoverEvents, event)
		}
	}
	if err := rows.Err(); err != nil {
		return err
...
func scanUsageLog(scanner interface{ Scan(...any) error }) (*service.UsageLog, error) {
	...
	log := &service.UsageLog{
		...
		UserPaygDiscountMultiplier:    &userPaygDiscountMultiplier,
...
func appendSessionIDWhereCondition(conditions []string, args []any, column, sessionID string) ([]string, []any) {
	sessionID = strings.TrimSpace(sessionID)
	if sessionID == "" {
		return conditions, args
	}
	condition, arg := buildSessionIDFilterCondition(column, len(args)+1, sessionID)
	conditions = append(conditions, condition)
	args = append(args, arg)
	return conditions, args
}

func appendAccountSwitchWhereCondition(conditions []string, args []any, table string, filters UsageLogFilters) ([]string, []any) {
	if !filters.AccountSwitch {
		return conditions, args
	}
	...
	subConditions := []string{
		fmt.Sprintf("sw.session_id = %s.session_id", table),
		fmt.Sprintf("sw.api_key_id = %s.api_key_id", table),
		fmt.Sprintf("COALESCE(sw.group_id, 0) = COALESCE(%s.group_id, 0)", table),
	}
	if filters.StartTime != nil {
		subConditions = append(subConditions, fmt.Sprintf("sw.created_at >= $%d", len(args)+1))
		args = append(args, *filters.StartTime)
	}
	if filters.EndTime != nil {
		subConditions = append(subConditions, fmt.Sprintf("sw.created_at < $%d", len(args)+1))
	...
	condition := fmt.Sprintf(`%[1]s.session_id IS NOT NULL AND %[1]s.session_id <> '' AND EXISTS (
  SELECT 1
  FROM usage_logs sw
  WHERE %[2]s
  GROUP BY sw.api_key_id, COALESCE(sw.group_id, 0), sw.session_id
  HAVING COUNT(DISTINCT sw.account_id) > 1
)`, table, strings.Join(subConditions, " AND "))
	conditions = append(conditions, condition)
	return conditions, args
}

func appendSessionIDQueryFilter(query string, args []any, column, sessionID string) (string, []any) {
	sessionID = strings.TrimSpace(sessionID)
	if sessionID == "" {
		return query, args
	}
...
func buildSessionIDFilterCondition(column string, argIndex int, sessionID string) (string, any) {
	if isFullUUIDSessionID(sessionID) {
		return fmt.Sprintf("%s = $%d", column, argIndex), sessionID
	}
	return fmt.Sprintf(`%s LIKE $%d ESCAPE '\'`, column, argIndex), escapeSQLLikePrefix(sessionID) + "%"
}

func isFullUUIDSessionID(value string) bool {
	if len(value) != 36 {
		return false
	}
