...
var (
	...
	updateSubUsageScript = redis.NewScript(`
		...
		local window_start = tonumber(redis.call('HGET', KEYS[1], 'five_hour_window_start') or 0)
		if window_start == 0 or (now - window_start) >= 18000 then
...
func (c *billingCache) parseSubscriptionCache(data map[string]string) (*service.SubscriptionCacheData, error) {
	...
	if fiveHourWindowStr, ok := data[subFieldFiveHourWindowStart]; ok {
		if unix, err := strconv.ParseInt(fiveHourWindowStr, 10, 64); err == nil && unix > 0 {
			t := time.Unix(unix, 0)
			result.FiveHourWindowStart = &t
		}
	}
...
func (c *billingCache) SetSubscriptionCache(ctx context.Context, userID, groupID int64, data *service.SubscriptionCacheData) error {
	...
	if data.FiveHourWindowStart != nil {
		fields[subFieldFiveHourWindowStart] = data.FiveHourWindowStart.Unix()
	}
