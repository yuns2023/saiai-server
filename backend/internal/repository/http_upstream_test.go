...
func TestDecodeUpstreamResponse_Gzip(t *testing.T) {
	...
	require.NoError(t, decodeUpstreamResponse(resp))
...
func TestDecodeUpstreamResponse_Brotli(t *testing.T) {
	var buf bytes.Buffer
	zw := brotli.NewWriter(&buf)
	_, err := zw.Write([]byte("hello-br"))
	require.NoError(t, err)
	require.NoError(t, zw.Close())
	...
	body, err := io.ReadAll(resp.Body)
	require.NoError(t, err)
	require.Equal(t, "hello-br", string(body))
	require.Empty(t, resp.Header.Get("Content-Encoding"))
}

func TestDecodeUpstreamResponse_Zstd(t *testing.T) {
	var buf bytes.Buffer
	...
	body, err := io.ReadAll(resp.Body)
	require.NoError(t, err)
	require.Equal(t, "hello-zstd", string(body))
	require.Empty(t, resp.Header.Get("Content-Encoding"))
}

func TestDecodeUpstreamResponse_Deflate(t *testing.T) {
	var buf bytes.Buffer
	...
	body, err := io.ReadAll(resp.Body)
	require.NoError(t, err)
	require.Equal(t, "hello-deflate", string(body))
	require.Empty(t, resp.Header.Get("Content-Encoding"))
}

func TestDecodeUpstreamResponse_MultipleEncodings(t *testing.T) {
	var gzipBuf bytes.Buffer
	gzipWriter := gzip.NewWriter(&gzipBuf)
	_, err := gzipWriter.Write([]byte("hello-multi"))
	require.NoError(t, err)
	require.NoError(t, gzipWriter.Close())

	var brBuf bytes.Buffer
	brWriter := brotli.NewWriter(&brBuf)
	_, err = brWriter.Write(gzipBuf.Bytes())
	require.NoError(t, err)
	...
	resp := &http.Response{
		...
		Body: io.NopCloser(bytes.NewReader(brBuf.Bytes())),
	}

	require.NoError(t, decodeUpstreamResponse(resp))
	body, err := io.ReadAll(resp.Body)
	require.NoError(t, err)
	require.Equal(t, "hello-multi", string(body))
}

func TestDecodeUpstreamResponse_UnsupportedEncoding(t *testing.T) {
	resp := &http.Response{
		Header: http.Header{
	...
	err := decodeUpstreamResponse(resp)
	require.Error(t, err)
	require.Contains(t, err.Error(), "unsupported upstream content-encoding")
}
