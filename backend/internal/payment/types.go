...
// RefundRejectedError means the provider returned a definitive business
// rejection and no refund occurred. Transport, timeout, malformed-response,
// and HTTP 5xx errors must not use this type because their outcome is
// ambiguous and must be queried or resolved manually.
...
func NewRefundRejectedError(err error) error {
	if err == nil {
		return nil
	}
	return &RefundRejectedError{Err: err}
}

func IsRefundRejectedError(err error) bool {
...
type Provider interface {
	...
	// Currency is the settlement currency expected by CreatePayment.
	Currency() string
...
// RefundProvider is an optional capability. Payment adapters that do not
// support refunds can still participate in order creation and fulfillment.
