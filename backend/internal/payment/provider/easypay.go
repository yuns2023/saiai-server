...
func NewEasyPay(instanceID string, config map[string]string) (*EasyPay, error) {
	...
	for _, key := range []string{"apiBase", "notifyUrl", "returnUrl"} {
		parsed, err := url.Parse(strings.TrimSpace(config[key]))
		if err != nil || parsed.Host == "" || (parsed.Scheme != "https" && parsed.Scheme != "http") {
			return nil, fmt.Errorf("easypay config key %s must be an absolute HTTP URL", key)
		}
	}
	...
	if err != nil {
		return nil, fmt.Errorf("easypay config currency: %w", err)
	}
...
func (e *EasyPay) Currency() string {
...
func (e *EasyPay) NotificationOrderID(rawBody string, _ map[string]string) (string, error) {
	values, err := url.ParseQuery(rawBody)
	if err != nil {
		return "", fmt.Errorf("parse easypay notification order ID: %w", err)
	}
	orderID := strings.TrimSpace(values.Get("out_trade_no"))
	if orderID == "" {
		return "", fmt.Errorf("easypay notification is missing out_trade_no")
	}
	return orderID, nil
}
...
func parseEasyPayRefundResponse(status int, body []byte) error {
	...
	if !easyPayResponseCodeIsSuccess(resp.Code) {
		...
		return payment.NewRefundRejectedError(fmt.Errorf("easypay refund failed (HTTP %d): %s", status, msg))
	}
	return nil
}
