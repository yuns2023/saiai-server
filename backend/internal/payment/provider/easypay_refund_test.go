...
func TestEasyPayRefundBusinessRejectionIsDefinitive(t *testing.T) {
	t.Parallel()
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		_, _ = w.Write([]byte(`{"code":0,"msg":"refund disabled"}`))
	}))
	defer server.Close()

	provider := newTestEasyPay(t, server.URL)
	_, err := provider.Refund(context.Background(), payment.RefundRequest{OrderID: "out-456", Amount: "1.50"})
	if !payment.IsRefundRejectedError(err) {
		t.Fatalf("Refund error = %v, want definitive rejection", err)
	}
}
