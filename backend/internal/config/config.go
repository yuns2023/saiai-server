...
type GatewayConfig struct {
	...
	// OpenAI 图片生成/编辑响应体读取上限（字节）。图片 base64 响应通常明显大于文本响应。
	OpenAIImagesResponseReadMaxBytes int64 `mapstructure:"openai_images_response_read_max_bytes"`
	// 代理探测响应体读取上限（字节）
	...
	// StickySessionTTLSeconds: Anthropic/Gemini 通用 session_hash -> account_id 粘连 TTL（秒）
	...
	// AnthropicStreamTerminalGraceSeconds: 收到 Anthropic 最终 message_delta 后等待 message_stop 的宽限时间（秒），0表示禁用
	AnthropicStreamTerminalGraceSeconds int `mapstructure:"anthropic_stream_terminal_grace_seconds"`
...
func setDefaults() {
	...
	// 如果将来切到自建镜像/官方镜像，必须同时更新这里和 fallback 同步任务。
	...
	viper.SetDefault("gateway.openai_images_response_read_max_bytes", int64(64*1024*1024))
	...
	viper.SetDefault("gateway.sticky_session_ttl_seconds", 7200)
	viper.SetDefault("gateway.stream_data_interval_timeout", 180)
	viper.SetDefault("gateway.anthropic_stream_terminal_grace_seconds", 10)
...
func (c *Config) Validate() error {
	...
	if c.Gateway.OpenAIImagesResponseReadMaxBytes <= 0 {
		return fmt.Errorf("gateway.openai_images_response_read_max_bytes must be positive")
	}
	...
	if c.Gateway.StickySessionTTLSeconds <= 0 {
		return fmt.Errorf("gateway.sticky_session_ttl_seconds must be positive")
	}
	...
	if c.Gateway.AnthropicStreamTerminalGraceSeconds < 0 {
		return fmt.Errorf("gateway.anthropic_stream_terminal_grace_seconds must be non-negative")
	}
	if c.Gateway.AnthropicStreamTerminalGraceSeconds > 60 {
		return fmt.Errorf("gateway.anthropic_stream_terminal_grace_seconds must be 0 or between 1-60 seconds")
	}
