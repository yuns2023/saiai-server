...
func TestLoadAnthropicStreamTerminalGraceConfig(t *testing.T) {
	t.Run("default", func(t *testing.T) {
		...
		require.Equal(t, 10, cfg.Gateway.AnthropicStreamTerminalGraceSeconds)
	})

	t.Run("environment override", func(t *testing.T) {
		resetViperWithJWTSecret(t)
		t.Setenv("GATEWAY_ANTHROPIC_STREAM_TERMINAL_GRACE_SECONDS", "7")

		cfg, err := Load()
		require.NoError(t, err)
		require.Equal(t, 7, cfg.Gateway.AnthropicStreamTerminalGraceSeconds)
	})

	t.Run("zero disables recovery", func(t *testing.T) {
		resetViperWithJWTSecret(t)

		cfg, err := Load()
		require.NoError(t, err)
		cfg.Gateway.AnthropicStreamTerminalGraceSeconds = 0
...
func TestLoadDefaultOpenAIWSConfig(t *testing.T) {
	...
	if cfg.Gateway.StickySessionTTLSeconds != 7200 {
		t.Fatalf("Gateway.StickySessionTTLSeconds = %d, want 7200", cfg.Gateway.StickySessionTTLSeconds)
	}
...
func TestValidateConfigErrors(t *testing.T) {
	...
	cases := []struct {
		...
		{
			name:    "gateway anthropic stream terminal grace range",
			mutate:  func(c *Config) { c.Gateway.AnthropicStreamTerminalGraceSeconds = 61 },
			wantErr: "gateway.anthropic_stream_terminal_grace_seconds",
		},
		{
			name:    "gateway anthropic stream terminal grace negative",
			mutate:  func(c *Config) { c.Gateway.AnthropicStreamTerminalGraceSeconds = -1 },
			wantErr: "gateway.anthropic_stream_terminal_grace_seconds must be non-negative",
		},
		{
