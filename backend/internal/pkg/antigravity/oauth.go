...
// defaultClientSecret 故意为空。公开仓库不分发第三方 OAuth client_secret；
// 运营者必须通过 ANTIGRAVITY_OAUTH_CLIENT_SECRET 提供有权使用的凭据。
...
func getClientSecret() (string, error) {
	...
	return "", infraerrors.Newf(http.StatusBadRequest, "ANTIGRAVITY_OAUTH_CLIENT_SECRET_MISSING", "Antigravity compatibility OAuth is disabled; configure an authorized credential with %s", AntigravityOAuthClientSecretEnv)
}

// ValidateOAuthClientConfiguration fails before an authorization session is
// created when the compatibility client has not been explicitly configured.
func ValidateOAuthClientConfiguration() error {
