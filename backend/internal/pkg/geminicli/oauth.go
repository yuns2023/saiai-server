...
//
// If ClientID/ClientSecret is not provided, the compatibility client is used
// only when GeminiCLIOAuthClientSecretEnv is explicitly configured.
//
...
func EffectiveOAuthConfig(cfg OAuthConfig, oauthType string) (OAuthConfig, error) {
	...
	// The compatibility client is disabled unless the operator supplies a
	// credential they are authorized to use. No third-party client secret is
	...
	if effective.ClientID == "" && effective.ClientSecret == "" {
		...
		if secret == "" {
			return OAuthConfig{}, infraerrors.Newf(http.StatusBadRequest, "GEMINI_CLI_OAUTH_CLIENT_SECRET_MISSING", "Gemini CLI compatibility OAuth is disabled; configure an authorized credential with %s or provide a custom OAuth client", GeminiCLIOAuthClientSecretEnv)
		}
		effective.ClientID = GeminiCLIOAuthClientID
		effective.ClientSecret = secret
	} else if effective.ClientID == "" || effective.ClientSecret == "" {
		return OAuthConfig{}, infraerrors.New(http.StatusBadRequest, "GEMINI_OAUTH_CLIENT_NOT_CONFIGURED", "OAuth client not configured: set both client_id and client_secret, or leave both empty and configure an authorized compatibility credential")
	}
