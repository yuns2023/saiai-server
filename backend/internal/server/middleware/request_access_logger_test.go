...
func TestRequestLogger_DoesNotExposeLocalRequestIDOnGatewayCompatRoutes(t *testing.T) {
	gin.SetMode(gin.TestMode)
	r := gin.New()
	r.Use(RequestLogger())
	...
	if got := w.Header().Get(requestIDHeader); got != "" {
		t.Fatalf("gateway-compatible route should not expose local request id, got %q", got)
	}
}

func TestRequestLogger_AllowsUpstreamRequestIDOnGatewayCompatRoutes(t *testing.T) {
	gin.SetMode(gin.TestMode)
	r := gin.New()
	r.Use(RequestLogger())
	r.POST("/v1/messages", func(c *gin.Context) {
		c.Header("x-request-id", "req_01upstream")
		c.Status(http.StatusOK)
	})
	...
	if got := w.Header().Get("x-request-id"); got != "req_01upstream" {
		t.Fatalf("expected upstream request id passthrough, got %q", got)
	}
}
