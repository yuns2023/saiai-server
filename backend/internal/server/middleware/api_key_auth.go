...
func apiKeyAuthWithSubscription(apiKeyService *service.APIKeyService, subscriptionService *service.SubscriptionService, inputModeration *service.InputModerationService, cfg *config.Config) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		// Disabling a group must stop every API-key route for already-issued
		// keys as well as new key bindings. In particular, do not let usage or
		// bootstrap metadata remain available for a disabled group.
		...
		// ── 7. 计费检查通过 → 写入 subscription context + TouchLastUsed → Next ─
