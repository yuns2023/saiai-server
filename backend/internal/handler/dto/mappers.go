...
func groupFromServiceBase(g *service.Group) Group {
	return Group{
		...
		ClaudeEnvironmentMode:           g.EffectiveClaudeEnvironmentMode(),
		ClaudeEnvironmentRewrite:        g.EffectiveClaudeEnvironmentMode() == service.ClaudeEnvironmentModeRewrite,
...
func AccountFromServiceShallow(a *service.Account) *Account {
	...
	out := &Account{
		...
		PaygDiscountMultiplier:  a.PaygDiscountRate(),
		Status:                  a.Status,
	...
	if a.IsAnthropicOAuthOrSetupToken() {
		...
		mode := a.GetClaudeOAuthMode()
		out.ClaudeOAuthMode = &mode
		if threshold := a.GetClaudeOAuth5hRateLimitThresholdPercent(); threshold > 0 {
			out.ClaudeOAuth5hRateLimitPercent = &threshold
		}
		if mode == service.ClaudeOAuthModeCarpool {
			...
			autoExpand := a.IsClaudeOAuthCarpoolAutoExpandEnabled()
			out.ClaudeOAuthCarpoolAutoExpand = &autoExpand
			autoMaintenanceTarget := a.GetClaudeOAuthCarpoolAutoMaintenanceTarget()
			out.ClaudeOAuthCarpoolAutoMaintenanceTarget = &autoMaintenanceTarget
		}
...
func usageLogFromServiceUser(l *service.UsageLog) UsageLog {
	...
	return UsageLog{
		...
		ReasoningEffortInherited:      l.ReasoningEffortInherited,
		...
		CacheCreation5mCost:           l.CacheCreation5mCost,
		CacheCreation1hCost:           l.CacheCreation1hCost,
...
func UsageLogFromServiceAdmin(l *service.UsageLog) *AdminUsageLog {
	...
	return &AdminUsageLog{
		...
		SessionAccountCount:   l.SessionAccountCount,
		SessionAccounts:       SessionAccountSummariesFromService(l.SessionAccounts),
		FailoverEvents:        FailoverEventsFromService(l.FailoverEvents),
	}
}

func SessionAccountSummariesFromService(accounts []service.UsageLogSessionAccount) []AccountSummary {
	if len(accounts) == 0 {
		return nil
	}
...
func FailoverEventsFromService(events []service.UsageLogFailoverEvent) []FailoverEvent {
	if len(events) == 0 {
		return nil
	}
	out := make([]FailoverEvent, 0, len(events))
	for _, event := range events {
		out = append(out, FailoverEvent{
			SourceAccount: AccountSummary{
				ID:   event.SourceAccountID,
				Name: event.SourceAccountName,
			},
