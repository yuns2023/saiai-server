...
func TestOpenAIEnsureResponsesDependencies(t *testing.T) {
	t.Run("missing_dependencies_returns_503", func(t *testing.T) {
		...
		assert.Equal(t, "gateway_dependencies_missing", errorObj["code"])
...
func TestWriteError_NonStreamingEmitsCodeAndRetryAfter(t *testing.T) {
	...
	h := &OpenAIGatewayHandler{}
	h.writeError(c, errEnvelopeUpstreamUnreachable, false)
	...
	errorObj, ok := parsed["error"].(map[string]any)
	require.True(t, ok)
	assert.Equal(t, "upstream_unreachable", errorObj["type"])
	assert.Equal(t, "upstream_request_failed", errorObj["code"])
	assert.Equal(t, "Upstream request failed", errorObj["message"])
}

func TestWriteError_OmitsCodeWhenEnvelopeHasNone(t *testing.T) {
	gin.SetMode(gin.TestMode)
	w := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(w)
	c.Request = httptest.NewRequest(http.MethodPost, "/v1/responses", nil)
	...
	assert.False(t, hasCode, "code field must be omitted when envelope.Code is empty")
	assert.Empty(t, w.Header().Get("Retry-After"))
}

func TestWriteError_StreamingEmitsSSEWithoutHeaderMutation(t *testing.T) {
	gin.SetMode(gin.TestMode)
	w := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(w)
	c.Request = httptest.NewRequest(http.MethodPost, "/v1/responses", nil)
	...
	h := &OpenAIGatewayHandler{}
	h.writeError(c, errEnvelopeUpstreamUnreachable, true)

	// Headers cannot be set after stream starts; Retry-After must be suppressed.
	assert.Empty(t, w.Header().Get("Retry-After"))
