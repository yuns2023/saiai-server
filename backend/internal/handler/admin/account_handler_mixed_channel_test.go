...
func TestAccountHandlerCheckMixedChannelIgnoresClaudeOAuthModes(t *testing.T) {
	adminSvc := newStubAdminService()
	router := setupAccountMixedChannelRouter(adminSvc)
	...
	body, _ := json.Marshal(map[string]any{
		"platform":  "anthropic",
		"type":      "oauth",
		"extra":     map[string]any{"claude_oauth_mode": "single_device", "account_uuid": "11111111-2222-4333-8444-555555555555"},
		"group_ids": []int64{27},
	})
	rec := httptest.NewRecorder()
	req := httptest.NewRequest(http.MethodPost, "/api/v1/admin/accounts/check-mixed-channel", bytes.NewReader(body))
