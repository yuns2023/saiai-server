...
func (h *AccountHandler) Create(c *gin.Context) {
	...
	if req.PaygDiscountMultiplier != nil {
		response.BadRequest(c, "payg_discount_multiplier is a user setting; configure it in user management")
		return
	}
...
// ListClaudeCarpoolDevices handles listing recorded/overflow carpool devices for an account.
// GET /api/v1/admin/accounts/:id/claude-carpool-devices
func (h *AccountHandler) ListClaudeCarpoolDevices(c *gin.Context) {
	accountID, err := strconv.ParseInt(c.Param("id"), 10, 64)
	if err != nil {
		response.BadRequest(c, "Invalid account ID")
		return
	...
	if !account.IsAnthropicOAuthOrSetupToken() {
		response.BadRequest(c, "Claude OAuth device management is only supported for Anthropic OAuth accounts")
		return
	}
	if h.identityService == nil {
		response.Success(c, &service.CarpoolDeviceOverview{})
		return
	}
	devices, err := h.identityService.ListCarpoolDevices(c.Request.Context(), account)
	if err != nil {
...
func (h *AccountHandler) DeleteClaudeCarpoolDevice(c *gin.Context) {
	...
	if err := h.identityService.DeleteCarpoolDevice(c.Request.Context(), account, deviceKey); err != nil {
		response.Error(c, http.StatusInternalServerError, err.Error())
		return
	}
	response.Success(c, gin.H{"message": "carpool device deleted"})
}

// ListClaudeSharedBuckets handles listing shared Claude OAuth buckets for an account.
// GET /api/v1/admin/accounts/:id/claude-shared-buckets
func (h *AccountHandler) ListClaudeSharedBuckets(c *gin.Context) {
	accountID, err := strconv.ParseInt(c.Param("id"), 10, 64)
	if err != nil {
		response.BadRequest(c, "Invalid account ID")
		return
	...
	if !account.IsAnthropicOAuthOrSetupToken() {
		response.BadRequest(c, "Claude shared buckets are only supported for Anthropic OAuth accounts")
		return
	}
	if h.identityService == nil {
		response.Success(c, gin.H{"items": []*service.SharedBucketInfo{}})
		return
	}
	buckets, err := h.identityService.ListSharedBuckets(c.Request.Context(), account)
	if err != nil {
		response.Error(c, http.StatusInternalServerError, err.Error())
		return
	}
	response.Success(c, gin.H{"items": buckets})
}

// DeleteClaudeSharedBucket handles deleting a shared Claude OAuth bucket.
// DELETE /api/v1/admin/accounts/:id/claude-shared-buckets/:bucket
func (h *AccountHandler) DeleteClaudeSharedBucket(c *gin.Context) {
	accountID, err := strconv.ParseInt(c.Param("id"), 10, 64)
	if err != nil {
		response.BadRequest(c, "Invalid account ID")
		return
	...
	if err != nil || bucket < 0 {
		response.BadRequest(c, "Invalid bucket ID")
		return
	}
	account, err := h.adminService.GetAccount(c.Request.Context(), accountID)
	if err != nil {
	...
	if err := h.identityService.DeleteSharedBucket(c.Request.Context(), account, bucket); err != nil {
		response.Error(c, http.StatusInternalServerError, err.Error())
		return
	}
	response.Success(c, gin.H{"message": "shared bucket deleted"})
}

// ListClaudeSingleDeviceSlots handles listing single_device Claude OAuth slots for an account.
// GET /api/v1/admin/accounts/:id/claude-single-device-slots
func (h *AccountHandler) ListClaudeSingleDeviceSlots(c *gin.Context) {
	accountID, err := strconv.ParseInt(c.Param("id"), 10, 64)
	if err != nil {
		response.BadRequest(c, "Invalid account ID")
		return
	...
	if !account.IsAnthropicOAuthOrSetupToken() || account.GetClaudeOAuthMode() != service.ClaudeOAuthModeSingleDevice {
		response.Success(c, gin.H{"items": []*service.SingleDeviceSlotInfo{}})
		return
	}
	if h.identityService == nil {
		response.Success(c, gin.H{"items": []*service.SingleDeviceSlotInfo{}})
	...
	items, err := h.identityService.ListSingleDeviceSlots(c.Request.Context(), account)
	if err != nil {
		response.Error(c, http.StatusInternalServerError, err.Error())
		return
	}
...
// GetClaudePinnedBinding handles showing the current pinned binding for an account.
// GET /api/v1/admin/accounts/:id/claude-pinned-binding
func (h *AccountHandler) GetClaudePinnedBinding(c *gin.Context) {
	accountID, err := strconv.ParseInt(c.Param("id"), 10, 64)
	if err != nil {
		response.BadRequest(c, "Invalid account ID")
		return
	...
	if !account.IsAnthropicOAuthOrSetupToken() || account.GetClaudeOAuthMode() != service.ClaudeOAuthModePinned {
		response.BadRequest(c, "Claude pinned bindings are only supported for Anthropic OAuth pinned accounts")
		return
	}
	if h.identityService == nil {
		response.Success(c, gin.H{"item": nil})
		return
	}
	groupID := int64(0)
	if len(account.GroupIDs) > 0 {
		groupID = account.GroupIDs[0]
	}
	if groupID <= 0 {
		response.Success(c, gin.H{"item": nil})
		return
	}
	item, err := h.identityService.GetPinnedAccountBinding(c.Request.Context(), groupID, account)
	if err != nil {
		response.Error(c, http.StatusInternalServerError, err.Error())
		return
	}
...
// DeleteClaudePinnedBinding handles clearing the current pinned binding for an account.
// DELETE /api/v1/admin/accounts/:id/claude-pinned-binding
func (h *AccountHandler) DeleteClaudePinnedBinding(c *gin.Context) {
	accountID, err := strconv.ParseInt(c.Param("id"), 10, 64)
	if err != nil {
		response.BadRequest(c, "Invalid account ID")
		return
	...
	if groupID <= 0 {
		response.Success(c, gin.H{"message": "no pinned binding"})
		return
	}
	if err := h.identityService.DeletePinnedAccountBinding(c.Request.Context(), groupID, account); err != nil {
		response.Error(c, http.StatusInternalServerError, err.Error())
		return
	}
	response.Success(c, gin.H{"message": "pinned binding deleted"})
}
...
func (h *AccountHandler) BatchCreate(c *gin.Context) {
	...
	executeAdminIdempotentJSON(c, "admin.accounts.batch_create", req, service.DefaultWriteIdempotencyTTL(), func(ctx context.Context) (any, error) {
		...
		for _, item := range req.Accounts {
			if item.PaygDiscountMultiplier != nil {
				failed++
				results = append(results, gin.H{
					"name":    item.Name,
					"success": false,
					"error":   "payg_discount_multiplier is a user setting; configure it in user management",
				})
				continue
			}
...
func (h *AccountHandler) GetAvailableModels(c *gin.Context) {
	...
	// Handle OpenAI accounts: 透传哲学下不再做账号级模型映射，统一返回默认模型集。
	if account.IsOpenAI() {
