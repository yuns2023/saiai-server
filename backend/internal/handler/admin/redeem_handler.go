...
type GenerateRedeemCodesRequest struct {
	...
	Type         string  `json:"type" binding:"required,oneof=balance concurrency subscription invitation claude_device"`
...
type CreateAndRedeemCodeRequest struct {
	...
	Type         string  `json:"type" binding:"omitempty,oneof=balance concurrency subscription invitation claude_device"` // 不传时默认 balance（向后兼容）
...
func (h *RedeemHandler) CreateAndRedeem(c *gin.Context) {
	...
	// 向后兼容：旧版调用方（如 SAiAi Pay）不传 type 字段，默认当作 balance 充值处理。
	...
	if req.Type == "claude_device" && req.GroupID == nil {
		response.BadRequest(c, "group_id is required for Claude device type")
		return
	}
