	...
	for {
		...
		if err != nil {
			...
			if len(failedAccountIDs) == 0 {
				h.writeNoAvailableAccount(c, streamStarted)
				return
			}
...
// writeNoAvailableAccount emits the standard gateway-side "no candidate
// account" error: 503 with type=gateway_error / code=no_available_account.
// Distinct from upstream_error (upstream replied 4xx/5xx) so monitoring and
// runbook can branch on cause without parsing message strings.
func (h *SoraGatewayHandler) writeNoAvailableAccount(c *gin.Context, streamStarted bool) {
	body := gin.H{
		"error": gin.H{
			"type":    "gateway_error",
