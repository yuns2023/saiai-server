...
func attachOpsRequestBodyToEntry(c *gin.Context, entry *service.OpsInsertErrorLogInput, opsSvc ...*service.OpsService) {
	if c == nil || entry == nil {
		return
	}
	...
	var ops *service.OpsService
	if len(opsSvc) > 0 {
		ops = opsSvc[0]
	}
	if opts, ok := service.GetOpsRequestBodyCaptureOptions(c); ok && opts.MaxBytes > 0 {
		entry.RequestBodyJSON, entry.RequestBodyTruncated, entry.RequestBodyBytes = service.PrepareOpsRequestBodyForQueueWithLimit(raw, opts.MaxBytes)
	} else if limit, enabled := opsFullRequestBodyLimit(c, ops); enabled {
		entry.RequestBodyJSON, entry.RequestBodyTruncated, entry.RequestBodyBytes = service.PrepareOpsRequestBodyForQueueWithLimit(raw, limit)
	} else {
...
func opsFullRequestBodyLimit(c *gin.Context, ops *service.OpsService) (int, bool) {
	...
	return ops.FullRequestBodyLoggingLimit(c.Request.Context(), apiKey.ID)
}

func recordOpsRequestCaptureForError(c *gin.Context, ops *service.OpsService, entry *service.OpsInsertErrorLogInput) {
	if c == nil || ops == nil || entry == nil || c.Request == nil {
		return
	}
	apiKey, ok := middleware2.GetAPIKeyFromContext(c)
	if !ok || apiKey == nil || apiKey.ID <= 0 {
	...
	opts, ok := ops.RequestBodyCaptureOptions(c.Request.Context(), apiKey.ID)
	if !ok {
		return
	}
	...
	input := &service.OpsInsertRequestCaptureInput{
		Outcome:            "error",
		RequestID:          firstNonEmpty(entry.RequestID, c.Writer.Header().Get("x-request-id"), c.Writer.Header().Get("X-Request-Id")),
		ClientRequestID:    firstNonEmpty(entry.ClientRequestID, gatewayClientRequestID(c)),
		UserID:             entry.UserID,
		...
		UpstreamEndpoint:   GetUpstreamEndpoint(c, entry.Platform),
		Stream:             entry.Stream,
		SessionID:          strings.TrimSpace(c.GetHeader("X-Claude-Code-Session-Id")),
		RequestPayloadHash: opsRequestPayloadHash(c),
		UserAgent:          entry.UserAgent,

		StatusCode:         &statusCode,
		UpstreamStatusCode: entry.UpstreamStatusCode,
		ErrorMessage:       stringPtrIfNotBlank(entry.ErrorMessage),

		InboundRequestHeadersJSON: entry.RequestHeadersJSON,
		InboundRequestBody:        entry.RequestBodyJSON,
		InboundRequestTruncated:   entry.RequestBodyTruncated,
		InboundRequestBytes:       entry.RequestBodyBytes,
	...
	if input.UserID == nil && apiKey.User != nil {
		input.UserID = &apiKey.User.ID
	}
	if input.GroupID == nil && apiKey.GroupID != nil {
	...
	if input.InboundRequestHeadersJSON == nil {
		input.InboundRequestHeadersJSON = marshalOpsCapturedHeaders(service.CaptureOpsRequestHeaders(c.Request))
	}

	if opts.CaptureUpstream {
		if snapshot, ok := service.GetOpsUpstreamForwardRequestSnapshot(c); ok && snapshot != nil {
			input.UpstreamRequestMethod = snapshot.Method
			input.UpstreamRequestURL = snapshot.URL
			input.UpstreamRequestHeadersJSON = marshalOpsCapturedHeaders(snapshot.Headers)
			...
			input.UpstreamRequestBody = upstreamBodyJSON
			input.UpstreamRequestTruncated = upstreamTruncated
			input.UpstreamRequestBytes = upstreamBodyBytes
		}
		if responseSnapshot, ok := service.GetOpsUpstreamForwardResponseSnapshot(c); ok && responseSnapshot != nil {
			if responseSnapshot.StatusCode > 0 {
				input.UpstreamStatusCode = &responseSnapshot.StatusCode
			}
			input.RequestID = firstNonEmpty(input.RequestID, opsCapturedHeaderValue(responseSnapshot.Headers, "x-request-id", "request-id", "anthropic-request-id"))
			input.UpstreamResponseHeadersJSON = marshalOpsCapturedHeaders(responseSnapshot.Headers)
			...
			input.UpstreamResponseBody = responseBodyJSON
			input.UpstreamResponseTruncated = responseTruncated
			input.UpstreamResponseBytes = responseBodyBytes
		} else if entry.UpstreamErrorDetail != nil && strings.TrimSpace(*entry.UpstreamErrorDetail) != "" {
			responseBodyJSON, responseTruncated, responseBodyBytes := service.PrepareOpsRequestBodyForCaptureWithLimit([]byte(*entry.UpstreamErrorDetail), opts.MaxBytes)
			input.UpstreamResponseBody = responseBodyJSON
			input.UpstreamResponseTruncated = responseTruncated
			input.UpstreamResponseBytes = responseBodyBytes
		} else if strings.TrimSpace(entry.ErrorBody) != "" {
			responseBodyJSON, responseTruncated, responseBodyBytes := service.PrepareOpsRequestBodyForCaptureWithLimit([]byte(entry.ErrorBody), opts.MaxBytes)
			input.UpstreamResponseBody = responseBodyJSON
			input.UpstreamResponseTruncated = responseTruncated
			input.UpstreamResponseBytes = responseBodyBytes
		}
	...
	logOpsErrorRequestBodyCapture(input)

	go func() {
		ctx, cancel := context.WithTimeout(context.Background(), opsErrorLogTimeout)
		defer cancel()
		_ = ops.RecordRequestCapture(ctx, input)
	}()
}

func logOpsErrorRequestBodyCapture(input *service.OpsInsertRequestCaptureInput) {
	if input == nil || (input.InboundRequestBody == nil && input.UpstreamRequestBody == nil) {
		return
	}
	...
	fields := []zap.Field{
		...
		zap.String("platform", strings.TrimSpace(input.Platform)),
		zap.String("model", strings.TrimSpace(input.Model)),
		zap.String("request_path", strings.TrimSpace(input.RequestPath)),
		...
		zap.String("session_id", strings.TrimSpace(input.SessionID)),
		zap.String("request_payload_hash", strings.TrimSpace(input.RequestPayloadHash)),
		zap.String("user_agent", strings.TrimSpace(input.UserAgent)),
		zap.Bool("request_body_truncated", input.InboundRequestTruncated),
		zap.Bool("upstream_request_body_truncated", input.UpstreamRequestTruncated),
		zap.Bool("upstream_response_body_truncated", input.UpstreamResponseTruncated),
	}
	if input.UserID != nil {
	...
	if input.AccountID != nil {
		fields = append(fields, zap.Int64("account_id", *input.AccountID))
	}
	if input.GroupID != nil {
		fields = append(fields, zap.Int64("group_id", *input.GroupID))
	}
	if input.StatusCode != nil {
		fields = append(fields, zap.Int("status_code", *input.StatusCode))
	}
	if input.UpstreamStatusCode != nil {
		fields = append(fields, zap.Int("upstream_status_code", *input.UpstreamStatusCode))
	}
	if input.ErrorMessage != nil {
		fields = append(fields, zap.String("error_message", strings.TrimSpace(*input.ErrorMessage)))
	}
	if input.InboundRequestHeadersJSON != nil {
		fields = append(fields, zap.Any("inbound_request_headers", opsCapturedHeadersJSONForLog(*input.InboundRequestHeadersJSON)))
	}
	if input.InboundRequestBytes != nil {
		fields = append(fields, zap.Int("request_body_bytes", *input.InboundRequestBytes))
	}
	if input.InboundRequestBody != nil {
		fields = append(fields, zap.String("request_body", *input.InboundRequestBody))
	}
	if strings.TrimSpace(input.UpstreamRequestMethod) != "" {
		fields = append(fields, zap.String("upstream_request_method", strings.TrimSpace(input.UpstreamRequestMethod)))
	}
	if strings.TrimSpace(input.UpstreamRequestURL) != "" {
		fields = append(fields, zap.String("upstream_request_url", strings.TrimSpace(input.UpstreamRequestURL)))
	}
	if input.UpstreamRequestHeadersJSON != nil {
		fields = append(fields, zap.Any("upstream_request_headers", opsCapturedHeadersJSONForLog(*input.UpstreamRequestHeadersJSON)))
	}
	if input.UpstreamRequestBytes != nil {
		fields = append(fields, zap.Int("upstream_request_body_bytes", *input.UpstreamRequestBytes))
	}
	if input.UpstreamRequestBody != nil {
		fields = append(fields, zap.String("upstream_request_body", *input.UpstreamRequestBody))
	}
	if input.UpstreamResponseHeadersJSON != nil {
		fields = append(fields, zap.Any("upstream_response_headers", opsCapturedHeadersJSONForLog(*input.UpstreamResponseHeadersJSON)))
	}
	if input.UpstreamResponseBytes != nil {
		fields = append(fields, zap.Int("upstream_response_body_bytes", *input.UpstreamResponseBytes))
	}
	if input.UpstreamResponseBody != nil {
		fields = append(fields, zap.String("upstream_response_body", *input.UpstreamResponseBody))
	}

	logger.L().Info("gateway error request body captured", fields...)
}

func opsCapturedHeadersJSONForLog(raw string) any {
	raw = strings.TrimSpace(raw)
	if raw == "" {
		return nil
	}
	var headers []service.OpsCapturedHeaderLine
...
func opsRequestPayloadHash(c *gin.Context) string {
	if c == nil {
		return ""
	}
	v, ok := c.Get(opsRequestBodyKey)
	...
	return service.HashUsageRequestPayload(raw)
}
...
func extractOpsRequestHeadersForLog(c *gin.Context, ops *service.OpsService) *string {
	if c == nil || c.Request == nil {
		return nil
	}
	if shouldCaptureFullOpsRequestHeaders(c, ops) {
		headers := service.CaptureOpsRequestHeaders(c.Request)
		if len(headers) > 0 {
			raw, err := json.Marshal(headers)
			if err == nil {
				s := string(raw)
	...
	return extractOpsRetryRequestHeaders(c)
}

func shouldCaptureFullOpsRequestHeaders(c *gin.Context, ops *service.OpsService) bool {
	if service.ShouldCaptureFullOpsRequestHeaders(c) {
		return true
	}
	_, enabled := opsFullRequestBodyLimit(c, ops)
	return enabled
}
