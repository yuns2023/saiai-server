...
func (m *sessionWindowMockRepo) SetRateLimited(_ context.Context, id int64, resetAt time.Time) error {
	m.rateLimitCalls = append(m.rateLimitCalls, rlCall{ID: id, ResetAt: resetAt})
	return nil
}
func (m *sessionWindowMockRepo) SetModelRateLimit(_ context.Context, id int64, scope string, resetAt time.Time) error {
	m.modelRateLimitCalls = append(m.modelRateLimitCalls, mrlCall{ID: id, Scope: scope, ResetAt: resetAt})
	return nil
}
...
func TestUpdateSessionWindow_RateLimitsClaudeOAuthWhen5hThresholdReached(t *testing.T) {
	resetUnix := time.Now().Add(2 * time.Hour).Unix()
	wantReset := time.Unix(resetUnix, 0).UTC()
	repo := &sessionWindowMockRepo{}
	svc := newRateLimitServiceForTest(repo)
	...
	account := &Account{
		...
		Extra: map[string]any{
			"claude_oauth_5h_rate_limit_threshold_percent": 90.0,
		},
	}
	headers := http.Header{}
	headers.Set("anthropic-ratelimit-unified-5h-status", "allowed_warning")
	...
	call := repo.rateLimitCalls[0]
	if call.ID != account.ID {
		t.Fatalf("expected account ID %d, got %d", account.ID, call.ID)
	}
	if !call.ResetAt.Equal(wantReset) {
		t.Fatalf("expected reset %v, got %v", wantReset, call.ResetAt)
	}
}

func TestUpdateSessionWindow_DoesNotRateLimitClaudeOAuthBelow5hThreshold(t *testing.T) {
	resetUnix := time.Now().Add(2 * time.Hour).Unix()
	repo := &sessionWindowMockRepo{}
	svc := newRateLimitServiceForTest(repo)
	...
	headers.Set("anthropic-ratelimit-unified-5h-status", "allowed")
	headers.Set("anthropic-ratelimit-unified-5h-reset", fmt.Sprintf("%d", resetUnix))
	headers.Set("anthropic-ratelimit-unified-5h-utilization", "0.89")

	svc.UpdateSessionWindow(context.Background(), account, headers)

	if len(repo.rateLimitCalls) != 0 {
		t.Fatalf("expected no SetRateLimited calls below threshold, got %d", len(repo.rateLimitCalls))
	}
}

func TestUpdateSessionWindow_DoesNotRateLimitClaudeOAuthWithout5hReset(t *testing.T) {
	repo := &sessionWindowMockRepo{}
	svc := newRateLimitServiceForTest(repo)
	...
	if len(repo.rateLimitCalls) != 0 {
		t.Fatalf("expected no SetRateLimited calls without authoritative reset, got %d", len(repo.rateLimitCalls))
	}
}
