	...
	return NewBillingService(&config.Config{}, &PricingService{
		pricingData: map[string]*LiteLLMModelPricing{
			...
			"gpt-5.2": {
				...
				InputCostPerTokenPriority:       3.5e-6,
				OutputCostPerToken:              14e-6,
				OutputCostPerTokenPriority:      28e-6,
				CacheCreationInputTokenCost:     1.75e-6,
				CacheReadInputTokenCost:         0.175e-6,
				CacheReadInputTokenCostPriority: 0.35e-6,
			},
			"gpt-5.2-codex": {
				InputCostPerToken:               1.75e-6,
				InputCostPerTokenPriority:       3.5e-6,
...
func TestGetModelPricing_OpenAIGPT51DynamicPricing(t *testing.T) {
	svc := newOpenAITestBillingService()
...
func TestGetModelPricing_OpenAIGPT54DynamicPricing(t *testing.T) {
	svc := newOpenAITestBillingService()
...
func TestGetModelPricing_OpenAIGPT54MiniDynamicPricing(t *testing.T) {
	svc := newOpenAITestBillingService()
...
func TestGetModelPricing_OpenAIGPT54NanoDynamicPricing(t *testing.T) {
	svc := newOpenAITestBillingService()
...
func TestCalculateCost_OpenAILongContextThresholdIncludesCacheWrites(t *testing.T) {
	pricingSvc := &PricingService{pricingData: map[string]*LiteLLMModelPricing{
		"gpt-5.6-sol": {
			...
			LongContextInputCostMultiplier:         2.0,
			LongContextOutputCostMultiplier:        1.5,
			LongContextCacheCreationCostMultiplier: 2.0,
			LongContextCacheReadCostMultiplier:     2.0,
		},
	}}
	svc := NewBillingService(&config.Config{}, pricingSvc)
	tokens := UsageTokens{InputTokens: 260000, CacheCreationTokens: 13000, OutputTokens: 100}
...
func TestCalculateCost_OpenAILongContextModelsApplyOfficialWholeRequestPricing(t *testing.T) {
	pricingSvc := &PricingService{pricingData: map[string]*LiteLLMModelPricing{
		"gpt-5.4-pro":   {InputCostPerToken: 30e-6, OutputCostPerToken: 180e-6, CacheReadInputTokenCost: 3e-6},
		"gpt-5.5":       {InputCostPerToken: 5e-6, OutputCostPerToken: 30e-6, CacheReadInputTokenCost: 0.5e-6},
		"gpt-5.5-pro":   {InputCostPerToken: 30e-6, OutputCostPerToken: 180e-6, CacheReadInputTokenCost: 3e-6},
		"gpt-5.6-sol":   {InputCostPerToken: 5e-6, OutputCostPerToken: 30e-6, CacheCreationInputTokenCost: 6.25e-6, CacheCreationInputTokenCostPriority: 12.5e-6, CacheReadInputTokenCost: 0.5e-6, InputCostPerTokenPriority: 10e-6, OutputCostPerTokenPriority: 60e-6, CacheReadInputTokenCostPriority: 1e-6},
	...
	svc := NewBillingService(&config.Config{}, pricingSvc)
	tokens := UsageTokens{InputTokens: 272000, CacheReadTokens: 1, CacheCreationTokens: 50, OutputTokens: 100}
	...
	tests := []struct {
		...
		{"gpt-5.4-pro-2026-03-05", 30e-6, 3e-6, 180e-6},
		{"gpt-5.5-2026-04-23", 5e-6, 0.5e-6, 30e-6},
		{"gpt-5.5-pro-2026-04-23", 30e-6, 30e-6, 180e-6},
		{"gpt-5.6-sol", 5e-6, 0.5e-6, 30e-6},
		{"gpt-5.6-terra", 2.5e-6, 0.25e-6, 15e-6},
		{"gpt-5.6-luna", 1e-6, 0.1e-6, 6e-6},
	}
	...
	for _, tt := range tests {
		t.Run(tt.model, func(t *testing.T) {
			...
			require.NoError(t, err)
			require.InDelta(t, float64(tokens.InputTokens)*tt.input*2, cost.InputCost, 1e-10)
			require.InDelta(t, float64(tokens.CacheReadTokens)*tt.cacheRead*2, cost.CacheReadCost, 1e-10)
	...
	require.InDelta(t, float64(tokens.CacheCreationTokens)*12.5e-6, priorityCost.CacheCreationCost, 1e-10)
	...
	gpt55PriorityCost, err := svc.CalculateCostWithServiceTier("gpt-5.5-2026-04-23", tokens, 1.0, "priority")
	require.NoError(t, err)
	require.InDelta(t, float64(tokens.InputTokens)*12.5e-6, gpt55PriorityCost.InputCost, 1e-10)
	require.InDelta(t, float64(tokens.CacheReadTokens)*1.25e-6, gpt55PriorityCost.CacheReadCost, 1e-10)
	require.InDelta(t, float64(tokens.OutputTokens)*75e-6, gpt55PriorityCost.OutputCost, 1e-10)

	standardCost, err := svc.CalculateCost("gpt-5.6-sol", tokens, 1.0)
	require.NoError(t, err)
	require.InDelta(t, float64(tokens.CacheCreationTokens)*6.25e-6*2, standardCost.CacheCreationCost, 1e-10)
}
...
func TestCalculateCost_FableUsesOneHourCachePricing(t *testing.T) {
	svc := newTestBillingService()

	tokens := UsageTokens{
		InputTokens:           1808,
		OutputTokens:          30,
		CacheCreationTokens:   17713,
		CacheCreation1hTokens: 17713,
	}
	cost, err := svc.CalculateCost("claude-fable-5", tokens, 1.0)
	require.NoError(t, err)

	require.InDelta(t, 1808*10e-6, cost.InputCost, 1e-12)
	require.InDelta(t, 30*50e-6, cost.OutputCost, 1e-12)
	require.InDelta(t, 17713*20e-6, cost.CacheCreationCost, 1e-12)
	require.InDelta(t, 0.37384, cost.TotalCost, 1e-12)
}

func TestGetModelPricing_IgnoresZeroDynamicPricingWhenFallbackExists(t *testing.T) {
	...
	pricing, err := svc.GetModelPricing("claude-mythos-5")
	require.NoError(t, err)
...
func TestBillingServiceGetModelPricing_OpenAIDynamicGpt52Variants(t *testing.T) {
	svc := newOpenAITestBillingService()
...
func TestGetModelPricing_OpenAIGpt52DynamicPricingExposesPriorityPrices(t *testing.T) {
	svc := newOpenAITestBillingService()
