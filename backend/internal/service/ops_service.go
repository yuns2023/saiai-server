...
// PrepareOpsRequestBodyForQueueWithLimit is the scoped troubleshooting variant
// of PrepareOpsRequestBodyForQueue. It still redacts sensitive JSON keys, but
// allows selected API keys to keep a larger body sample while debugging.
...
// PrepareOpsRequestBodyForCaptureWithLimit keeps JSON object field order for
// troubleshooting captures. It is intentionally used for opt-in success
// captures, not retry payloads, because truncation may return a non-JSON prefix.
func PrepareOpsRequestBodyForCaptureWithLimit(raw []byte, maxBytes int) (requestBodyJSON *string, truncated bool, requestBodyBytes *int) {
	if len(raw) == 0 {
		return nil, false, nil
	}
	if maxBytes <= 0 {
	...
	sanitized, truncated, bytesLen := sanitizeAndTrimRequestBodyPreserveOrder(raw, maxBytes)
	if sanitized != "" {
		out := sanitized
		requestBodyJSON = &out
	}
...
func (s *OpsService) RecordRequestCapture(ctx context.Context, input *OpsInsertRequestCaptureInput) error {
	if input == nil {
		return nil
	}
	...
	input.Platform = truncateString(strings.TrimSpace(input.Platform), 32)
	input.Model = truncateString(strings.TrimSpace(input.Model), 100)
	input.RequestPath = truncateString(strings.TrimSpace(input.RequestPath), 256)
	input.InboundEndpoint = truncateString(strings.TrimSpace(input.InboundEndpoint), 128)
	input.UpstreamEndpoint = truncateString(strings.TrimSpace(input.UpstreamEndpoint), 128)
	input.SessionID = truncateString(strings.TrimSpace(input.SessionID), 128)
	input.RequestPayloadHash = truncateString(strings.TrimSpace(input.RequestPayloadHash), 128)
	input.UpstreamRequestMethod = truncateString(strings.TrimSpace(input.UpstreamRequestMethod), 16)
	input.UserAgent = strings.TrimSpace(input.UserAgent)
	if input.ErrorMessage != nil {
		msg := truncateString(sanitizeUpstreamErrorMessage(strings.TrimSpace(*input.ErrorMessage)), 2048)
		if msg == "" {
	...
	if _, err := s.opsRepo.InsertRequestCapture(ctx, input); err != nil {
		log.Printf("[Ops] RecordRequestCapture failed: %v", err)
		return err
	}
	return nil
}
...
func sanitizeAndTrimRequestBodyPreserveOrder(raw []byte, maxBytes int) (jsonString string, truncated bool, bytesLen int) {
	bytesLen = len(raw)
	if len(raw) == 0 {
		return "", false, 0
	}
	...
	encoded, err := redactJSONPreserveOrder(raw)
	if err != nil {
		return "", false, bytesLen
	}
	if len(encoded) <= maxBytes {
		return encoded, false, bytesLen
	}

	const marker = "\n...[TRUNCATED]"
	if maxBytes <= len(marker) {
		return marker, true, bytesLen
	}
	return encoded[:maxBytes-len(marker)] + marker, true, bytesLen
}

func redactJSONPreserveOrder(raw []byte) (string, error) {
	dec := json.NewDecoder(bytes.NewReader(raw))
	dec.UseNumber()
	var b strings.Builder
	if err := writeJSONValuePreserveOrder(dec, &b); err != nil {
		return "", err
	}
...
func writeJSONValuePreserveOrder(dec *json.Decoder, b *strings.Builder) error {
	tok, err := dec.Token()
	if err != nil {
		return err
	}
	return writeJSONTokenPreserveOrder(dec, b, tok)
}

func writeJSONTokenPreserveOrder(dec *json.Decoder, b *strings.Builder, tok any) error {
	switch t := tok.(type) {
	case json.Delim:
		switch t {
		case '{':
