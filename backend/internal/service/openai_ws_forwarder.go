...
func parseOpenAIWSResponseUsageFromCompletedEvent(message []byte, usage *OpenAIUsage) {
	...
	usage.CacheCreationInputTokens = int(values[2].Int())
	usage.CacheReadInputTokens = int(values[3].Int())
	usage.ReportedServiceTier = extractOpenAIReportedServiceTierFromBody(message)
}
...
func populateOpenAIUsageFromResponseJSON(body []byte, usage *OpenAIUsage) {
	...
	usage.CacheCreationInputTokens = int(values[2].Int())
	usage.CacheReadInputTokens = int(values[3].Int())
	usage.ReportedServiceTier = extractOpenAIReportedServiceTierFromBody(body)
}
...
func (s *OpenAIGatewayService) selectAccountByPreviousResponseID(
	...
	if err != nil || account == nil {
		// Keep the ownership record until its TTL expires. The caller must not
		// send this provider-issued continuation to another account merely
