...
// groupFilteredOpenAIAccountRepo models the repository's authoritative group
// query; unlike the general stub, it does not return accounts removed from the
// requested group.
type groupFilteredOpenAIAccountRepo struct{ stubOpenAIAccountRepo }

func (r groupFilteredOpenAIAccountRepo) ListSchedulableByGroupIDAndPlatform(_ context.Context, groupID int64, platform string) ([]Account, error) {
	var result []Account
...
func TestOpenAISelectAccountWithLoadAwareness_StickyRemovedFromGroupClearsSession(t *testing.T) {
	ctx := context.Background()
	groupID := int64(45)
	sessionHash := "removed-from-group"
	oldAccount := Account{
	...
	cache := &stubGatewayCache{sessionBindings: map[string]int64{"openai:" + sessionHash: oldAccount.ID}}
	svc := &OpenAIGatewayService{
		accountRepo:        groupFilteredOpenAIAccountRepo{stubOpenAIAccountRepo{accounts: []Account{oldAccount, currentAccount}}},
		cache:              cache,
		concurrencyService: NewConcurrencyService(stubConcurrencyCache{}),
	}

	selection, err := svc.SelectAccountWithLoadAwareness(ctx, &groupID, sessionHash, "gpt-5.1", nil)
	require.NoError(t, err)
	require.NotNil(t, selection)
	require.NotNil(t, selection.Account)
	require.Equal(t, currentAccount.ID, selection.Account.ID)
...
// OpenAI account selection no longer filters by account-level model_mapping —
// the gateway is pass-through, the upstream decides whether it accepts the
// model. Account model_mapping still exists in DB but other platforms
// (anthropic/gemini/antigravity) keep using it; this test asserts the OpenAI
// path explicitly does NOT consult IsModelSupported.
func TestOpenAISelectAccountForModelWithExclusions_PassThroughIgnoresWhitelist(t *testing.T) {
	repo := stubOpenAIAccountRepo{
		accounts: []Account{
			{
				ID:          1,
	...
	// Account only lists gpt-3.5-turbo in its model_mapping, but a request for
	// gpt-4 must still pick this account — the upstream gets the chance to
	// reject (or honor) the request rather than the gateway second-guessing it.
...
// Sticky-session selection must keep working under pass-through: even when
// the requested model isn't in the account's model_mapping whitelist, an
// existing sticky binding should still resolve to the bound account.
func TestOpenAISelectAccountForModelWithExclusions_PassThroughHonorsStickyBinding(t *testing.T) {
	...
	cache := &stubGatewayCache{
		sessionBindings: map[string]int64{"openai:" + sessionHash: 7},
	}
	...
	// Account 7's whitelist only knows gpt-3.5-turbo, but the request asks for
	// gpt-4 — pass-through means whitelist no longer filters, and the sticky
	// binding must still pick account 7.
	acc, err := svc.SelectAccountForModelWithExclusions(context.Background(), nil, sessionHash, "gpt-4", nil)
	if err != nil {
		t.Fatalf("expected sticky hit, got error: %v", err)
	}
	if acc == nil || acc.ID != 7 {
		t.Fatalf("expected sticky-bound account 7, got %+v", acc)
	}
}
...
func TestNormalizeOpenAICompactRequestBodyPreservesFutureFields(t *testing.T) {
	body := []byte(`{"model":"gpt-5.3-codex","input":[{"type":"input_text","text":"hi"}],"previous_response_id":"resp_prev","prompt_cache_key":"pcache","future_compact_field":{"enabled":true},"stream":false}`)

	got, changed, err := NormalizeOpenAICompactRequestBodyForTest(body)
...
func TestOpenAIBuildUpstreamRequestCompactForcesJSONAcceptForOAuth(t *testing.T) {
	...
	c.Request.Header.Set("chatgpt-account-id", "client-spoof")
	c.Request.Header.Set("User-Agent", "codex-tui/0.128.0 (Ubuntu 22.04; x86_64)")
	c.Request.Header.Set("Version", "0.128.0")
...
func TestOpenAIBuildUpstreamRequestCompactDoesNotSynthesizeVersionForOAuth(t *testing.T) {
	gin.SetMode(gin.TestMode)
	rec := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(rec)
	c.Request = httptest.NewRequest(http.MethodPost, "/v1/responses/compact", bytes.NewReader([]byte(`{"model":"gpt-5.5"}`)))
	c.Request.Header.Set("User-Agent", "codex_vscode/0.131.0-alpha.9 (Windows 10.0.26100; x86_64) unknown (VS Code; 26.513.21555)")
...
func TestOpenAIBuildUpstreamRequestCompactPassthroughsClientVersionForOAuth(t *testing.T) {
	gin.SetMode(gin.TestMode)
	rec := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(rec)
	c.Request = httptest.NewRequest(http.MethodPost, "/v1/responses/compact", bytes.NewReader([]byte(`{"model":"gpt-5.5"}`)))
	c.Request.Header.Set("User-Agent", "codex_vscode/0.131.0-alpha.9 (Windows 10.0.26100; x86_64) unknown (VS Code; 26.513.21555)")
	c.Request.Header.Set("Version", "0.131.0-alpha.9")
	...
	req, err := svc.buildUpstreamRequest(c.Request.Context(), c, account, []byte(`{"model":"gpt-5.5"}`), "token", false, "", true)
	require.NoError(t, err)
	require.Equal(t, "0.131.0-alpha.9", req.Header.Get("Version"))
}

func TestOpenAIBuildUpstreamRequestCopiesHeadersByDenylist(t *testing.T) {
	gin.SetMode(gin.TestMode)
	rec := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(rec)
	c.Request = httptest.NewRequest(http.MethodPost, "/v1/responses/compact", bytes.NewReader([]byte(`{"model":"gpt-5"}`)))
	c.Request.Header.Set("OpenAI-Future-Feature", "preserve")
	...
	req, err := svc.buildUpstreamRequest(c.Request.Context(), c, account, []byte(`{"model":"gpt-5"}`), "upstream-key", false, "", true)
	require.NoError(t, err)
	require.Equal(t, "preserve", req.Header.Get("OpenAI-Future-Feature"))
	...
	require.Empty(t, req.Header.Get("CF-Connecting-IP"))
}
...
func TestOpenAIForwardCompactPreservesPreviousResponseIDAndFutureFields(t *testing.T) {
	...
	rec := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(rec)
	body := []byte(`{"model":"gpt-5.3-codex","stream":false,"previous_response_id":"resp_prev_compact","input":[{"type":"input_text","text":"summarize"}],"future_compact_field":{"mode":"new"}}`)
	...
	upstream := &httpUpstreamRecorder{
		resp: &http.Response{
			StatusCode: http.StatusOK,
			Header:     http.Header{"Content-Type": []string{"application/json"}},
			Body:       io.NopCloser(strings.NewReader(`{"id":"resp_compact_ok","model":"gpt-5.3-codex","usage":{"input_tokens":1,"output_tokens":1}}`)),
		},
	}
	svc := &OpenAIGatewayService{
		httpUpstream: upstream,
	...
	require.NoError(t, err)
	require.NotNil(t, result)
	require.Equal(t, "resp_prev_compact", gjson.GetBytes(upstream.lastBody, "previous_response_id").String())
	require.Equal(t, "new", gjson.GetBytes(upstream.lastBody, "future_compact_field.mode").String())
}
...
func TestParseSSEUsage_SelectiveParsing(t *testing.T) {
	...
	svc.parseSSEUsage(`{"type":"response.completed","response":{"usage":{"input_tokens":3,"output_tokens":5,"input_tokens_details":{"cache_write_tokens":1,"cached_tokens":2}}}}`, usage)
	...
	svc.parseSSEUsage(`{"type":"response.done","response":{"usage":{"input_tokens":13,"output_tokens":15,"input_tokens_details":{"cache_write_tokens":6,"cached_tokens":4}}}}`, usage)
