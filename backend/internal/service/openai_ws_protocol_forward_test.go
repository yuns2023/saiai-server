...
func TestOpenAIGatewayService_Forward_HTTPIngressOAuthPreservesPreviousResponseID(t *testing.T) {
	gin.SetMode(gin.TestMode)
		...
		responses: []*http.Response{
			{
				...
				Header:     http.Header{"Content-Type": []string{"application/json"}},
				Body: io.NopCloser(strings.NewReader(
					`{"error":{"code":"previous_response_not_found","type":"invalid_request_error","message":"previous response not found"}}`,
				)),
			},
			{
				StatusCode: http.StatusOK,
...
func TestOpenAIGatewayService_Forward_HTTPIngressOAuthKeepsPreviousResponseIDForToolOutput(t *testing.T) {
	...
	body := []byte(`{"model":"gpt-5.5","stream":false,"previous_response_id":"resp_tool_context","input":[{"type":"function_call_output","call_id":"call_1","output":"ok"}]}`)
	result, err := svc.Forward(context.Background(), c, account, body)
	require.Error(t, err)
	require.Nil(t, result)
	require.Equal(t, 1, upstream.callCount)
	require.Equal(t, "resp_tool_context", gjson.GetBytes(upstream.bodies[0], "previous_response_id").String())
	require.Equal(t, http.StatusBadRequest, rec.Code)
}

func TestOpenAIGatewayService_Forward_HTTPIngressPassesThroughInvalidRequest(t *testing.T) {
	gin.SetMode(gin.TestMode)
		...
		responses: []*http.Response{
			{
				...
				Header:     http.Header{"Content-Type": []string{"application/json"}},
				Body: io.NopCloser(strings.NewReader(
					`{"error":{"message":"No tool output found for tool search call fcyE1YpYdaLWLHC7oat05FYLCr.","type":"invalid_request_error","param":"input","code":null}}`,
				)),
			},
		},
	}
	...
	require.Contains(t, rec.Body.String(), "No tool output found for tool search call")
}
		...
		responses: []*http.Response{
			{
				...
				Header:     http.Header{"Content-Type": []string{"application/json"}},
				Body: io.NopCloser(strings.NewReader(
					`{"error":{"code":null,"message":"{\"error\":{\"message\":\"The encrypted content could not be verified.\",\"type\":\"invalid_request_error\",\"param\":null,\"code\":\"invalid_encrypted_content\"}}（traceid: 00000000000000000000000000000001）","param":null,"type":"invalid_request_error"}}`,
				)),
			},
			{
				StatusCode: http.StatusOK,
