...
// HandleUpstreamErrorForModel is the model-aware variant used by Anthropic
// messages forwarding paths. The legacy method intentionally stays available
// for auxiliary endpoints and other platforms where a request model is not
// meaningful for rate-limit persistence.
func (s *RateLimitService) HandleUpstreamErrorForModel(ctx context.Context, account *Account, statusCode int, headers http.Header, responseBody []byte, requestedModel string) (shouldDisable bool) {
	...
	case 429:
		s.handle429ForModel(ctx, account, headers, responseBody, requestedModel)
		shouldDisable = false
	case 529:
...
func (s *RateLimitService) handle429(ctx context.Context, account *Account, headers http.Header, responseBody []byte) {
	s.handle429ForModel(ctx, account, headers, responseBody, "")
}

func (s *RateLimitService) handle429ForModel(ctx context.Context, account *Account, headers http.Header, responseBody []byte, requestedModel string) {
	...
	if result := calculateAnthropic429ResetTime(headers); result != nil {
		if !result.anyWindowExceeded() {
			if s.handleAnthropicFableWindow429(ctx, account, requestedModel, headers) ||
				s.handleAnthropicModelScoped429(ctx, account, requestedModel, result) {
				return
			}
			if account.Platform == PlatformAnthropic {
				slog.Warn("anthropic_429_no_window_exceeded_skipped",
					"account_id", account.ID,
					"requested_model", strings.TrimSpace(requestedModel),
					"reset_at", result.resetAt.UTC(),
					"reason", "5h/7d headers did not indicate account-level exhaustion")
				return
			}
		}
	...
	if resetTimestamp == "" {
		...
		if account.Platform == PlatformAnthropic {
			if s.handleAnthropicCarpool5hQuotaWithoutReset(ctx, account, responseBody) {
				return
			}
...
func (s *RateLimitService) handleAnthropicModelScoped429(ctx context.Context, account *Account, requestedModel string, result *anthropic429Result) bool {
	...
	if result.fiveHourReset == nil || !result.fiveHourReset.After(time.Now()) {
		slog.Warn("anthropic_model_rate_limit_skipped",
			"account_id", account.ID,
			"model", model,
			"reason", "missing_or_expired_5h_reset")
		return true
	}

	resetAt := result.fiveHourReset.UTC()
	if err := s.accountRepo.SetModelRateLimit(ctx, account.ID, anthropicFableRateLimitKey, resetAt); err != nil {
		slog.Warn("anthropic_model_rate_limit_set_failed",
			"account_id", account.ID,
			"model", anthropicFableRateLimitKey,
			"reset_at", resetAt,
			"error", err)
		return true
	}
	slog.Warn("anthropic_model_rate_limited",
		"account_id", account.ID,
		"model", anthropicFableRateLimitKey,
		"reset_at", resetAt,
...
func (s *RateLimitService) handleAnthropicFableWindow429(ctx context.Context, account *Account, requestedModel string, headers http.Header) bool {
	...
	now := time.Now()
	resetAt, ok := parseAnthropicFutureReset(headers.Get("anthropic-ratelimit-unified-7d_oi-reset"), now, 8*24*time.Hour)
	if !ok {
		slog.Warn("anthropic_fable_window_rate_limit_skipped",
			"account_id", account.ID,
			"reason", "missing_or_invalid_7d_oi_reset")
		return true
	}
	...
	slog.Warn("anthropic_fable_window_model_rate_limited",
		"account_id", account.ID,
		"model", anthropicFableRateLimitKey,
		"reset_at", resetAt,
		"reset_in", time.Until(resetAt).Truncate(time.Second))
...
func parseAnthropicFutureReset(raw string, now time.Time, maxFuture time.Duration) (time.Time, bool) {
	...
	if !resetAt.After(now) || resetAt.After(now.Add(maxFuture)) {
		return time.Time{}, false
	}
	return resetAt, true
}
...
func (s *RateLimitService) handleAnthropicCarpool5hQuotaWithoutReset(ctx context.Context, account *Account, responseBody []byte) bool {
	...
	if !isAnthropicCarpool5hQuotaExhausted(responseBody) {
		return false
	}

	now := time.Now()
	resetAt, fromSessionWindow := anthropicCarpool5hQuotaFallbackResetAt(account, now)
	if account.RateLimitResetAt != nil && account.RateLimitResetAt.After(now) && !resetAt.After(*account.RateLimitResetAt) {
		slog.Warn("anthropic_carpool_5h_quota_existing_rate_limit_kept",
			"account_id", account.ID,
			"existing_reset_at", account.RateLimitResetAt.UTC(),
			"computed_reset_at", resetAt.UTC())
		return true
	}

	if err := s.accountRepo.SetRateLimited(ctx, account.ID, resetAt); err != nil {
		slog.Warn("anthropic_carpool_5h_quota_rate_limit_failed",
			"account_id", account.ID,
			"reset_at", resetAt.UTC(),
			"from_session_window", fromSessionWindow,
			"error", err)
		return true
	}

	if fromSessionWindow {
		windowEnd := resetAt
		windowStart := windowEnd.Add(-5 * time.Hour)
		if account.SessionWindowStart != nil && account.SessionWindowStart.Before(windowEnd) {
			windowStart = *account.SessionWindowStart
		}
		if err := s.accountRepo.UpdateSessionWindow(ctx, account.ID, &windowStart, &windowEnd, "rejected"); err != nil {
			slog.Warn("anthropic_carpool_5h_quota_session_window_update_failed",
				"account_id", account.ID,
				"reset_at", resetAt.UTC(),
				"error", err)
		}
	...
	slog.Warn("anthropic_carpool_5h_quota_rate_limited_without_reset",
		"account_id", account.ID,
		"reset_at", resetAt.UTC(),
		"reset_in", time.Until(resetAt).Truncate(time.Second),
		"from_session_window", fromSessionWindow)
...
func isAnthropicCarpool5hQuotaExhausted(responseBody []byte) bool {
	msg := strings.ToLower(strings.TrimSpace(extractUpstreamErrorMessage(responseBody)))
	return strings.Contains(msg, anthropicCarpool5hQuotaExhaustedMessage)
}

func anthropicCarpool5hQuotaFallbackResetAt(account *Account, now time.Time) (time.Time, bool) {
	if account != nil && account.SessionWindowEnd != nil {
		windowEnd := account.SessionWindowEnd.UTC()
		if windowEnd.After(now) && !windowEnd.After(now.Add(anthropicCarpool5hQuotaMaxWindowDuration)) {
			return windowEnd, true
		}
	}
	return now.Add(anthropicCarpool5hQuotaFallbackCooldown), false
}
...
type anthropic429Result struct {
	resetAt          time.Time  // The correct reset time to use for SetRateLimited when a window is exceeded
	fiveHourReset    *time.Time // 5h window reset timestamp (for session window calculation), nil if not available
	is5hExceeded     bool
	is7dExceeded     bool
	noWindowExceeded bool
}

func (r *anthropic429Result) anyWindowExceeded() bool {
	return r != nil && (r.is5hExceeded || r.is7dExceeded)
}
...
func calculateAnthropic429ResetTime(headers http.Header) *anthropic429Result {
	...
	return &anthropic429Result{
		...
		is5hExceeded:     is5hExceeded,
		is7dExceeded:     is7dExceeded,
		noWindowExceeded: !is5hExceeded && !is7dExceeded,
	}
}
...
func isAnthropicWindowExceeded(headers http.Header, window string) bool {
	...
	if surpassed := strings.TrimSpace(headers.Get(prefix + "surpassed-threshold")); surpassed != "" {
		if strings.EqualFold(surpassed, "true") {
			return true
		}
		if value, err := strconv.ParseFloat(surpassed, 64); err == nil && value >= 1.0-1e-9 {
			return true
		}
	}
...
func (s *RateLimitService) UpdateSessionWindow(ctx context.Context, account *Account, headers http.Header) {
	...
	if windowEnd != nil && needInitWindow {
		_ = s.accountRepo.UpdateExtra(ctx, account.ID, map[string]any{
			...
			"passive_usage_7d_sonnet_utilization": nil,
			"passive_usage_7d_sonnet_reset":       nil,
	...
	var fiveHourUtilizationPercent float64
	var hasFiveHourUtilization bool
	...
	if utilStr := headers.Get("anthropic-ratelimit-unified-5h-utilization"); utilStr != "" {
		if util, err := strconv.ParseFloat(utilStr, 64); err == nil {
			...
			fiveHourUtilizationPercent = normalizeUtilizationPercent(util)
			hasFiveHourUtilization = true
		}
	}
	...
	// 7d sonnet utilization（0-1 小数）
	if utilStr := headers.Get("anthropic-ratelimit-unified-7d-sonnet-utilization"); utilStr != "" {
		if util, err := strconv.ParseFloat(utilStr, 64); err == nil {
			extraUpdates["passive_usage_7d_sonnet_utilization"] = util
		}
	}
	// 7d sonnet reset timestamp
	if resetStr := headers.Get("anthropic-ratelimit-unified-7d-sonnet-reset"); resetStr != "" {
		if ts, err := strconv.ParseInt(resetStr, 10, 64); err == nil {
			if ts > 1e11 {
				ts = ts / 1000
			}
			extraUpdates["passive_usage_7d_sonnet_reset"] = ts
		}
	}
	addAnthropicFableUsageUpdates(extraUpdates, headers)
	if len(extraUpdates) > 0 {
		...
		if err := s.accountRepo.UpdateExtra(ctx, account.ID, extraUpdates); err != nil {
			...
			evalAccount := *account
			mergeAccountExtra(&evalAccount, extraUpdates)
			if hasFiveHourUtilization {
				setClaudeOAuth5hThresholdRateLimit(ctx, s.accountRepo, &evalAccount, fiveHourUtilizationPercent, fiveHourResetAt, "passive_response_header")
			}
		}
	}
...
func addAnthropicFableUsageUpdates(updates map[string]any, headers http.Header) {
	if updates == nil {
		return
	}
	...
	if resetStr := headers.Get("anthropic-ratelimit-unified-7d_oi-reset"); resetStr != "" {
		if resetAt, ok := parseAnthropicFutureReset(resetStr, time.Now(), 8*24*time.Hour); ok {
			updates["passive_usage_7d_oi_reset"] = resetAt.Unix()
		}
	}
}

func (s *RateLimitService) sampleAnthropicFableUsage(ctx context.Context, account *Account, headers http.Header) {
	...
	if err := s.accountRepo.UpdateExtra(ctx, account.ID, updates); err != nil {
		slog.Warn("anthropic_fable_passive_usage_update_failed", "account_id", account.ID, "error", err)
	}
}

func setClaudeOAuth5hThresholdRateLimit(ctx context.Context, repo AccountRepository, account *Account, utilizationPercent float64, resetAt *time.Time, source string) {
	if repo == nil || account == nil || account.Status != StatusActive {
		return
	}
	if !account.ShouldRateLimitForClaudeOAuth5hUsage(utilizationPercent) {
		return
	}
	threshold := account.GetClaudeOAuth5hRateLimitThresholdPercent()
	if resetAt == nil {
		slog.Warn("claude_oauth_5h_threshold_rate_limit_skipped", "account_id", account.ID, "reason", "missing_reset", "source", source, "utilization_percent", utilizationPercent, "threshold_percent", threshold)
		return
	}
	resetUTC := resetAt.UTC()
	now := time.Now()
	if !resetUTC.After(now) {
		slog.Warn("claude_oauth_5h_threshold_rate_limit_skipped", "account_id", account.ID, "reason", "reset_not_future", "source", source, "reset_at", resetUTC, "utilization_percent", utilizationPercent, "threshold_percent", threshold)
		return
	}
	if account.RateLimitResetAt != nil && now.Before(*account.RateLimitResetAt) && !resetUTC.After(account.RateLimitResetAt.UTC()) {
		return
	}
	if err := repo.SetRateLimited(ctx, account.ID, resetUTC); err != nil {
		slog.Warn("claude_oauth_5h_threshold_rate_limit_failed", "account_id", account.ID, "source", source, "reset_at", resetUTC, "utilization_percent", utilizationPercent, "threshold_percent", threshold, "error", err)
		return
	}
	slog.Warn("claude_oauth_5h_threshold_rate_limited", "account_id", account.ID, "source", source, "reset_at", resetUTC, "utilization_percent", utilizationPercent, "threshold_percent", threshold)
}
