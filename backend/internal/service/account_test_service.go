...
func generateSessionString(accountUUID string) (string, error) {
	b := make([]byte, 32)
	...
	return FormatMetadataUserID(hex64, strings.TrimSpace(accountUUID), sessionUUID, uaVersion), nil
}

// createTestPayload creates a Claude Code style test request payload
func createTestPayload(modelID string, includeBillingHeader bool, accountUUID string) (map[string]any, error) {
	sessionID, err := generateSessionString(accountUUID)
	if err != nil {
		return nil, err
	}

	systemBlocks := make([]map[string]any, 0, 2)
	if includeBillingHeader {
		if header, ok := buildClaudeBillingHeaderPlaceholder(claude.DefaultHeaders["User-Agent"], claude.DefaultHeaders["X-App"]); ok {
...
func (s *AccountTestService) testClaudeAccountConnection(c *gin.Context, account *Account, modelID string) error {
	...
	if account.IsOAuth() {
		...
		if account.Type != AccountTypeSetupToken && strings.TrimSpace(account.GetExtraString("account_uuid")) == "" {
			return s.sendErrorAndEnd(c, "No account UUID available")
		}
	...
	payload, err := createTestPayload(testModelID, account.IsOAuth(), oauthAccountUUID(account, account.GetExtraString("account_uuid")))
	if err != nil {
		return s.sendErrorAndEnd(c, "Failed to create test payload")
	}
	payloadBytes, _ := json.Marshal(payload)
	debugLogRequestBody("ACCOUNT_TEST_ORIGINAL", payloadBytes)
	if account.IsOAuth() {
		payloadBytes = rewriteClaudeBillingHeaderForUserAgent(payloadBytes, claude.DefaultHeaders["User-Agent"], 0)
	}
	debugLogRequestBody("ACCOUNT_TEST_UPSTREAM", payloadBytes)
	...
	debugLogRequestHeaders("ACCOUNT_TEST_UPSTREAM", req.Header)
