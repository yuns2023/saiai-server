...
func (p *ClaudeTokenProvider) GetAccessToken(ctx context.Context, account *Account) (string, error) {
	...
	if account.IsClaudeOAuthTokenSchedulingDisabled(time.Now()) {
		return "", errors.New("claude oauth token refresh disabled and token expires soon")
	}
	needsRefresh := !account.IsClaudeOAuthTokenRefreshDisabled() &&
		(expiresAt == nil || time.Until(*expiresAt) <= claudeTokenRefreshSkew)
	refreshFailed := false
