...
func newOpenAIRecordUsageBillingServiceForTest(cfg *config.Config) *BillingService {
...
func newOpenAIRecordUsageServiceForTest(usageRepo UsageLogRepository, userRepo UserRepository, subRepo UserSubscriptionRepository, rateRepo UserGroupRateRepository) *OpenAIGatewayService {
	...
	svc := NewOpenAIGatewayService(
		...
		nil,
		nil,
		newOpenAIRecordUsageBillingServiceForTest(cfg),
		nil,
		&BillingCacheService{},
		nil,
		&DeferredService{},
...
func TestOpenAIGatewayServiceRecordUsage_FailsClosedWhenPricingMissing(t *testing.T) {
	...
	err := svc.RecordUsage(context.Background(), &OpenAIRecordUsageInput{
		Result: &OpenAIForwardResult{
			RequestID: "openai_missing_pricing",
			Usage: OpenAIUsage{
				InputTokens:  10,
				OutputTokens: 6,
			},
			Model:    "gpt-5-unknown-preview",
			Duration: time.Second,
		},
...
func TestOpenAIGatewayServiceRecordUsage_Gpt56SolBillsCacheWritesSeparately(t *testing.T) {
	usageRepo := &openAIRecordUsageLogRepoStub{inserted: true}
	userRepo := &openAIRecordUsageUserRepoStub{}
	subRepo := &openAIRecordUsageSubRepoStub{}
	svc := newOpenAIRecordUsageServiceForTest(usageRepo, userRepo, subRepo, nil)
	...
	err := svc.RecordUsage(context.Background(), &OpenAIRecordUsageInput{
		Result: &OpenAIForwardResult{
			RequestID: "resp_gpt56_sol_cache_write",
			Usage: OpenAIUsage{
				InputTokens:              280000,
				OutputTokens:             100,
	...
	require.NoError(t, err)
	require.NotNil(t, usageRepo.lastLog)
	require.Equal(t, 250000, usageRepo.lastLog.InputTokens)
	require.Equal(t, 20000, usageRepo.lastLog.CacheCreationTokens)
	require.Equal(t, 10000, usageRepo.lastLog.CacheReadTokens)

	expectedInput := 250000 * 5e-6 * 2.0
	expectedCacheWrite := 20000 * 6.25e-6 * 2.0
	expectedCacheRead := 10000 * 0.5e-6 * 2.0
	expectedOutput := 100 * 30e-6 * 1.5
	expectedTotal := expectedInput + expectedCacheWrite + expectedCacheRead + expectedOutput
	require.InDelta(t, expectedInput, usageRepo.lastLog.InputCost, 1e-10)
	require.InDelta(t, expectedCacheWrite, usageRepo.lastLog.CacheCreationCost, 1e-10)
...
func TestExtractOpenAIUsageFromJSONBytes_CapturesReportedServiceTier(t *testing.T) {
	usage, ok := extractOpenAIUsageFromJSONBytes([]byte(`{
		...
		"usage":{"input_tokens":11,"output_tokens":7,"input_tokens_details":{"cache_write_tokens":4,"cached_tokens":3}}
	}`))

	require.True(t, ok)
	require.Equal(t, 4, usage.CacheCreationInputTokens)
	require.Equal(t, 3, usage.CacheReadInputTokens)
	require.NotNil(t, usage.ReportedServiceTier)
	require.Equal(t, "default", *usage.ReportedServiceTier)
}
...
func TestOpenAIGatewayServiceRecordUsage_UsesUpstreamModelWhenBillingModelMissing(t *testing.T) {
	usageRepo := &openAIRecordUsageLogRepoStub{inserted: true}
	billingRepo := &openAIRecordUsageBillingRepoStub{result: &UsageBillingApplyResult{Applied: true}}
	userRepo := &openAIRecordUsageUserRepoStub{}
	subRepo := &openAIRecordUsageSubRepoStub{}
	...
	require.NoError(t, err)
	require.NotNil(t, billingRepo.lastCmd)
	require.Equal(t, "gpt-5.1", billingRepo.lastCmd.Model)
	require.NotNil(t, usageRepo.lastLog)
	require.Equal(t, "alias-model", usageRepo.lastLog.Model)
	require.NotNil(t, usageRepo.lastLog.UpstreamModel)
