...
const (
	...
	openaiStickySessionTTL = time.Hour // OpenAI session sticky 上限
...
// or owned by the gateway.
var openaiDeniedRequestHeaders = map[string]struct{}{
	"authorization":       {},
	"cookie":              {},
	"host":                {},
	"content-length":      {},
...
type OpenAIUsage struct {
	...
	ReportedServiceTier      *string `json:"-"`
}
...
func (s *OpenAIGatewayService) HasModelPricing(model string) bool {
...
func resolveOpenAIBillingModel(result *OpenAIForwardResult) string {
	if result == nil {
		return ""
	}
...
func (s *OpenAIGatewayService) handleErrorResponse(
	...
	if resp.StatusCode == http.StatusBadRequest && (isOpenAIResponsesCompactPath(c) || isOpenAIInvalidRequestErrorBody(body)) {
		...
		errType := openAIUpstreamErrorType(body)
		if errType == "" {
			errType = "invalid_request_error"
		}
		c.JSON(http.StatusBadRequest, gin.H{
...
func isOpenAIInvalidRequestErrorBody(body []byte) bool {
	errType := strings.ToLower(strings.TrimSpace(openAIUpstreamErrorType(body)))
	if strings.Contains(errType, "invalid_request") {
		return true
	}

	// ChatGPT internal API style errors often use {"detail":"..."} for
	// non-retryable request/model validation failures. When the upstream status
	// is already 400, keep these as client-visible invalid_request_error instead
	// of mapping them to a retryable 502.
	return strings.TrimSpace(gjson.GetBytes(body, "detail").String()) != ""
}

func openAIUpstreamErrorType(body []byte) string {
...
func (s *OpenAIGatewayService) parseSSEUsageBytes(data []byte, usage *OpenAIUsage) {
	...
	usage.ReportedServiceTier = extractOpenAIReportedServiceTierFromBody(data)
}
...
func extractOpenAIUsageFromJSONBytes(body []byte) (OpenAIUsage, bool) {
	...
	return OpenAIUsage{
		...
		ReportedServiceTier:      extractOpenAIReportedServiceTierFromBody(body),
	}, true
}
...
func normalizeOpenAICompactRequestBody(body []byte) ([]byte, bool, error) {
	// Compact requests should track OpenAI's schema by default. Avoid
	// rebuilding the payload from a fixed field list; protocol-specific cleanup
	// happens later in applyCodexOAuthTransform where needed.
...
func (s *OpenAIGatewayService) RecordUsage(ctx context.Context, input *OpenAIRecordUsageInput) error {
	...
	// 计算实际的普通输入 token。OpenAI input_tokens 包含缓存读取和缓存写入明细，
	// 两者应分别按 cached input / cache write 单价计费，不能再次按普通输入计费。
	...
	if resolvedModel := resolveOpenAIBillingModel(result); resolvedModel != "" {
...
func deriveOpenAIReasoningEffortFromModel(model string) string {
	...
	return normalizeOpenAIReasoningEffortFromModelSuffix(parts[len(parts)-1])
}
...
func normalizeOpenAIReportedServiceTier(raw string) *string {
	value := strings.ToLower(strings.TrimSpace(raw))
	if value == "fast" {
		value = "priority"
	}
...
func extractOpenAIReportedServiceTierFromBody(body []byte) *string {
	if len(body) == 0 || !gjson.ValidBytes(body) {
		return nil
	}
	value := gjson.GetBytes(body, "service_tier")
	if !value.Exists() {
		value = gjson.GetBytes(body, "response.service_tier")
	}
	if !value.Exists() {
		return nil
	}
...
func normalizeOpenAIReasoningEffort(raw string) string {
	...
	case "xhigh", "extrahigh":
		return "xhigh"
	default:
		// Explicit request fields are telemetry, not validation; keep unknown
		// values so usage logs reflect what the client actually sent.
		return truncateString(value, 20)
	}
}

func normalizeOpenAIReasoningEffortFromModelSuffix(raw string) string {
	value := strings.ToLower(strings.TrimSpace(raw))
	if value == "" {
		return ""
	}
