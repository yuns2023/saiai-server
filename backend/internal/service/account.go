...
func (a *Account) IsSchedulable() bool {
	...
	if a.IsClaudeOAuthTokenSchedulingDisabled(now) {
		return false
	}
	return true
}
...
const (
	ClaudeOAuthModeCarpool                            = "carpool"
	ClaudeOAuthModeShared                             = "shared"
	...
	maxClaudeOAuthCarpoolDeviceLimit                  = 32
)

// GetClaudeOAuthMode returns the Claude OAuth traffic mode for Anthropic OAuth/SetupToken accounts.
// Defaults to carpool when unset or invalid.
func (a *Account) GetClaudeOAuthMode() string {
	if !a.IsAnthropicOAuthOrSetupToken() {
		return ""
	}
	switch strings.TrimSpace(strings.ToLower(a.getExtraString("claude_oauth_mode"))) {
...
// IsClaudeOAuthTokenRefreshDisabled 返回 Claude OAuth 账号是否禁用 token 自动刷新。
// single_device 模式默认禁用；显式配置 false 时可覆盖默认值。
func (a *Account) IsClaudeOAuthTokenRefreshDisabled() bool {
	if a == nil || !a.IsAnthropicOAuthOrSetupToken() {
		return false
	}
	if a.Extra != nil {
		if raw, ok := a.Extra["claude_oauth_disable_token_refresh"]; ok {
			if enabled, ok := raw.(bool); ok {
				return enabled
			}
		}
	}
	return a.GetClaudeOAuthMode() == ClaudeOAuthModeSingleDevice
}

// GetClaudeOAuthTokenDisableBeforeExpiryDuration 返回 token 临近过期时禁止调度的阈值。
// single_device 模式默认 3 分钟；<=0 视为关闭此保护。
func (a *Account) GetClaudeOAuthTokenDisableBeforeExpiryDuration() time.Duration {
	if a == nil || !a.IsAnthropicOAuthOrSetupToken() {
		return 0
	}
	if a.Extra != nil {
		if raw, ok := a.Extra["claude_oauth_token_disable_before_expiry_minutes"]; ok && raw != nil {
			minutes := parseExtraInt(raw)
			if minutes <= 0 {
				return 0
			}
	...
	if a.GetClaudeOAuthMode() == ClaudeOAuthModeSingleDevice {
		return time.Duration(DefaultClaudeOAuthTokenDisableBeforeExpiryMinutes) * time.Minute
	}
	return 0
}

// IsClaudeOAuthTokenSchedulingDisabled 返回 Claude OAuth 账号是否因 token 临近过期而禁止调度。
// 仅当“禁用自动刷新”开启时生效。
func (a *Account) IsClaudeOAuthTokenSchedulingDisabled(now time.Time) bool {
	if a == nil || !a.IsAnthropicOAuthOrSetupToken() {
		return false
	}
	if !a.IsClaudeOAuthTokenRefreshDisabled() {
		return false
	}
	threshold := a.GetClaudeOAuthTokenDisableBeforeExpiryDuration()
	if threshold <= 0 {
		return false
	}
	...
	return !now.Add(threshold).Before(*expiresAt)
}

// GetClaudeOAuth5hRateLimitThresholdPercent 返回 Claude OAuth/SetupToken 账号 5h 用量临时限流阈值（百分比）。
// <=0 视为关闭。触发后账号只会被标记到 5h reset 时间，不会被永久禁用。
func (a *Account) GetClaudeOAuth5hRateLimitThresholdPercent() float64 {
	if a == nil || !a.IsAnthropicOAuthOrSetupToken() {
		return 0
	}
	if a.Extra == nil {
	...
	raw, ok := a.Extra["claude_oauth_5h_rate_limit_threshold_percent"]
	if !ok || raw == nil {
		return 0
	}
	return parseExtraFloat64(raw)
}

// ShouldRateLimitForClaudeOAuth5hUsage 返回当前 5h utilization 是否达到临时限流阈值。
func (a *Account) ShouldRateLimitForClaudeOAuth5hUsage(utilizationPercent float64) bool {
	threshold := a.GetClaudeOAuth5hRateLimitThresholdPercent()
	if threshold <= 0 {
		return false
	}
...
// GetClaudeOAuthQuotaDisableThresholdPercent 返回旧版 Claude OAuth single_device 自动禁用阈值（百分比）。
// 该保护已废弃，默认关闭；保留解析函数仅兼容历史 extra 字段。
func (a *Account) GetClaudeOAuthQuotaDisableThresholdPercent() float64 {
	if a == nil || !a.IsAnthropicOAuthOrSetupToken() {
		return 0
	}
	if a.Extra != nil {
		if raw, ok := a.Extra["claude_oauth_quota_disable_threshold_percent"]; ok && raw != nil {
			return parseExtraFloat64(raw)
		}
	}
	return 0
...
func normalizeUtilizationPercent(raw any) float64 {
...
func (a *Account) getClaudeOAuthUsageUtilizationPercent(key string) float64 {
...
// GetClaudeOAuthObservedMaxUtilizationPercent 返回当前已缓存的最大额度使用率（百分比）。
func (a *Account) GetClaudeOAuthObservedMaxUtilizationPercent() float64 {
	if a == nil || !a.IsAnthropicOAuthOrSetupToken() {
		return 0
	}
	maxUtil := a.getClaudeOAuthUsageUtilizationPercent("session_window_utilization")
	if v := a.getClaudeOAuthUsageUtilizationPercent("passive_usage_7d_utilization"); v > maxUtil {
		maxUtil = v
	}
	if v := a.getClaudeOAuthUsageUtilizationPercent("passive_usage_7d_sonnet_utilization"); v > maxUtil {
		maxUtil = v
	}
	return maxUtil
}

// ShouldAutoDisableForClaudeOAuthQuota 返回是否应因额度达到阈值而自动禁用账号。
func (a *Account) ShouldAutoDisableForClaudeOAuthQuota() bool {
	threshold := a.GetClaudeOAuthQuotaDisableThresholdPercent()
	if threshold <= 0 {
		return false
	}
	return a.GetClaudeOAuthObservedMaxUtilizationPercent() >= threshold
}

// GetClaudeOAuthCarpoolDeviceLimit returns the configured carpool device limit for Anthropic OAuth accounts.
func (a *Account) GetClaudeOAuthCarpoolDeviceLimit() int {
	if !a.IsAnthropicOAuthOrSetupToken() {
		return 0
	}
	limit := a.getExtraInt("claude_oauth_carpool_device_limit")
	if limit <= 0 {
		limit = DefaultClaudeOAuthCarpoolDeviceLimit
	}
	if limit > maxClaudeOAuthCarpoolDeviceLimit {
...
func (a *Account) GetClaudeOAuthSharedBucketCount() int {
	...
	limit := a.getExtraInt("claude_oauth_shared_bucket_count")
	if limit <= 0 {
		limit = DefaultClaudeOAuthSharedBucketCount
	}
	if limit > maxClaudeOAuthSharedBucketCount {
		return maxClaudeOAuthSharedBucketCount
	}
	return limit
}

func (a *Account) GetClaudeOAuthFixedDeviceID() string {
	if !a.IsAnthropicOAuthOrSetupToken() {
		return ""
	}
	return strings.TrimSpace(a.getExtraString("claude_oauth_fixed_device_id"))
}

func (a *Account) GetClaudeOAuthFixedHeadersText() string {
	if !a.IsAnthropicOAuthOrSetupToken() {
		return ""
	}
...
// OAuth 账号默认启用，除非显式设置 enable_tls_fingerprint: false
