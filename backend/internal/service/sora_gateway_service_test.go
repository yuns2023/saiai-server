...
func TestParseSoraWatermarkOptions_NumericBool(t *testing.T) {
	...
	require.Equal(t, "custom", opts.ParseMethod)
}

func TestResolveWatermarkFreeURLRequiresExplicitCustomEndpoint(t *testing.T) {
	svc := &SoraGatewayService{}

	_, _, err := svc.resolveWatermarkFreeURL(context.Background(), nil, "generation-1", soraWatermarkOptions{ParseMethod: "third_party"})
	require.EqualError(t, err, "unsupported watermark parse method: third_party")

	_, _, err = svc.resolveWatermarkFreeURL(context.Background(), nil, "generation-1", soraWatermarkOptions{ParseMethod: "custom"})
	require.EqualError(t, err, "watermark_parse_url is required for custom watermark parsing")
}
