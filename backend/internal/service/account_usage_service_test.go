...
func TestEstimateSetupTokenUsageZerosExpiredSessionWindow(t *testing.T) {
	t.Parallel()
	...
	expiredUsage := svc.estimateSetupTokenUsageAt(&Account{
		SessionWindowEnd: &expired,
		Extra: map[string]any{
			"session_window_utilization": 0.56,
		},
	}, now)
	if expiredUsage.FiveHour == nil {
		t.Fatal("expected expired usage five_hour")
	}
	if expiredUsage.FiveHour.Utilization != 0 {
		t.Fatalf("expected expired session window utilization to be 0, got %v", expiredUsage.FiveHour.Utilization)
	}
	if expiredUsage.FiveHour.RemainingSeconds != 0 {
		t.Fatalf("expected expired session window remaining seconds to be 0, got %v", expiredUsage.FiveHour.RemainingSeconds)
	}

	activeUsage := svc.estimateSetupTokenUsageAt(&Account{
		SessionWindowEnd: &future,
		Extra: map[string]any{
			"session_window_utilization": 0.56,
		},
	}, now)
	if activeUsage.FiveHour == nil {
		t.Fatal("expected active usage five_hour")
	}
	if activeUsage.FiveHour.Utilization < 55.999 || activeUsage.FiveHour.Utilization > 56.001 {
