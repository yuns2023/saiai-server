...
func TestApplyCodexOAuthTransform_NormalizesToolSearchOutputCallIDWithCall(t *testing.T) {
	reqBody := map[string]any{
		"model":       "gpt-5.5",
		"tool_choice": "auto",
		"input": []any{
			map[string]any{"type": "tool_search_call", "call_id": "call_synthetic_tool_search_001", "status": "completed"},
			map[string]any{"type": "tool_search_output", "call_id": "call_synthetic_tool_search_001", "status": "completed"},
		},
	}
	...
	require.Equal(t, "fcsynthetic_tool_search_001", call["call_id"])
	require.Equal(t, "fcsynthetic_tool_search_001", output["call_id"])
}
...
func TestApplyCodexOAuthTransform_CompactForcesNonStreaming(t *testing.T) {
	...
	require.Equal(t, "resp_prev_123", reqBody["previous_response_id"])
	require.Equal(t, "pcache_123", reqBody["prompt_cache_key"])
	require.Equal(t, "keep-me", reqBody["future_compact_flag"])
...
// 透传哲学：normalizeCodexModel 不再做 alias 翻译，仅剥 provider 前缀。
// 客户端发什么模型名，原样上送 chatgpt 后端，由上游决定是否接受。
func TestNormalizeCodexModel_PassThroughKeepsModelAsIs(t *testing.T) {
	cases := map[string]string{
		...
		"vendor/foo/gpt-99": "gpt-99",
		// 空串保留空
		"": "",
	}

	for input, expected := range cases {
		require.Equal(t, expected, normalizeCodexModel(input), "input=%q", input)
	}
}
