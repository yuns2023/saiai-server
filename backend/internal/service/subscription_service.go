...
func (s *SubscriptionService) CheckAndActivateWindow(ctx context.Context, sub *UserSubscription) error {
	...
	if sub.FiveHourWindowStart == nil {
		fiveHourStart := time.Now()
		if err := s.userSubRepo.EnsureFiveHourWindow(ctx, sub.ID, fiveHourStart); err != nil && firstErr == nil {
			firstErr = err
		} else if err == nil {
			sub.FiveHourWindowStart = &fiveHourStart
			sub.FiveHourUsageUSD = 0
		}
	}
...
func (s *SubscriptionService) AdminResetQuota(ctx context.Context, subscriptionID int64, resetFiveHour, resetDaily, resetWeekly, resetMonthly bool) (*UserSubscription, error) {
	...
	windowStart := startOfDay(time.Now())
	if resetFiveHour {
		if err := s.userSubRepo.ResetFiveHourUsage(ctx, sub.ID, time.Now()); err != nil {
			return nil, err
		}
	}
	if resetDaily {
...
func (s *SubscriptionService) CheckAndResetWindows(ctx context.Context, sub *UserSubscription) error {
	...
	if sub.NeedsFiveHourReset() {
		fiveHourStart := time.Now()
		if err := s.userSubRepo.EnsureFiveHourWindow(ctx, sub.ID, fiveHourStart); err != nil {
			return err
		}
		sub.FiveHourWindowStart = &fiveHourStart
		sub.FiveHourUsageUSD = 0
