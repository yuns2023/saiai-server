...
func TestParseOpenAIWSResponseUsageFromCompletedEvent(t *testing.T) {
	...
	parseOpenAIWSResponseUsageFromCompletedEvent(
		[]byte(`{"type":"response.completed","response":{"service_tier":"default","usage":{"input_tokens":11,"output_tokens":7,"input_tokens_details":{"cache_write_tokens":4,"cached_tokens":3}}}}`),
		usage,
	)
...
func TestPopulateOpenAIUsageFromResponseJSONIncludesCacheWriteTokens(t *testing.T) {
	usage := &OpenAIUsage{}
	populateOpenAIUsageFromResponseJSON(
		[]byte(`{"service_tier":"flex","usage":{"input_tokens":13,"output_tokens":5,"input_tokens_details":{"cache_write_tokens":6,"cached_tokens":2}}}`),
		usage,
	)
	...
	require.NotNil(t, usage.ReportedServiceTier)
	require.Equal(t, "flex", *usage.ReportedServiceTier)
}
