...
type disconnectAfterWriteFrameConn struct {
...
func newDisconnectAfterWriteFrameConn() *disconnectAfterWriteFrameConn {
	return &disconnectAfterWriteFrameConn{
		base:      newPassthroughTestFrameConn(nil, false),
		writtenCh: make(chan struct{}),
	}
}

func (c *disconnectAfterWriteFrameConn) ReadFrame(ctx context.Context) (coderws.MessageType, []byte, error) {
	select {
	case <-ctx.Done():
		return coderws.MessageText, nil, ctx.Err()
	case <-c.writtenCh:
...
func (c *disconnectAfterWriteFrameConn) WriteFrame(ctx context.Context, msgType coderws.MessageType, payload []byte) error {
	if err := c.base.WriteFrame(ctx, msgType, payload); err != nil {
		return err
	}
	c.once.Do(func() { close(c.writtenCh) })
	return nil
}

func (c *disconnectAfterWriteFrameConn) Close() error {
	c.once.Do(func() { close(c.writtenCh) })
	return c.base.Close()
}
	...
	upstreamConn := newPassthroughTestFrameConn([]passthroughTestFrame{
		{
			...
			payload: []byte(`{"type":"response.completed","response":{"id":"resp_123","usage":{"input_tokens":7,"output_tokens":3,"input_tokens_details":{"cache_write_tokens":1,"cached_tokens":2}}}}`),
		},
	}, true)
...
func TestRelay_ClientDisconnectAfterForwardedTerminalIsGraceful(t *testing.T) {
	t.Parallel()

	clientConn := newDisconnectAfterWriteFrameConn()
	upstreamConn := newPassthroughTestFrameConn([]passthroughTestFrame{
		{
			msgType: coderws.MessageText,
			payload: []byte(`{"type":"response.completed","response":{"id":"resp_done","usage":{"input_tokens":2,"output_tokens":3}}}`),
		},
	}, false)
