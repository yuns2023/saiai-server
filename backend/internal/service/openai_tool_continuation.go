...
// RequiresPreviousResponseIDForToolContinuation reports whether the input
// contains continuation items that cannot safely be replayed after dropping the
// previous_response_id anchor.
func RequiresPreviousResponseIDForToolContinuation(reqBody map[string]any) bool {
	signals := AnalyzeToolContinuationSignals(reqBody)
	return signals.HasFunctionCallOutput || signals.HasItemReference
}
