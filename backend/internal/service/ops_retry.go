	...
	if errorLog != nil && strings.TrimSpace(errorLog.RequestHeaders) != "" {
		...
		if err := json.Unmarshal([]byte(errorLog.RequestHeaders), &stored); err == nil {
			for k, v := range stored {
				restoreOpsRetryHeader(req.Header, k, v)
			}
		} else {
			var ordered []OpsCapturedHeaderLine
			if err := json.Unmarshal([]byte(errorLog.RequestHeaders), &ordered); err == nil {
				for _, line := range ordered {
					restoreOpsRetryHeader(req.Header, line.Name, line.Value)
				}
			}
		}
	}
...
func restoreOpsRetryHeader(headers http.Header, key, value string) {
