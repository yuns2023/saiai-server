...
func TestAPIKeyService_InvalidateAuthCacheByKey_ClearsL1Immediately(t *testing.T) {
	var calls int32
	var currentGroupID atomic.Int64

	repo := &authRepoStub{
		getByKeyForAuth: func(ctx context.Context, key string) (*APIKey, error) {
			atomic.AddInt32(&calls, 1)
			var groupID *int64
			if gid := currentGroupID.Load(); gid > 0 {
				groupID = &gid
			}
			return &APIKey{
				ID:      21,
	...
	apiKey, err := svc.GetByKey(context.Background(), "k-l1-invalidate")
	require.NoError(t, err)
	require.Nil(t, apiKey.GroupID)
	svc.authCacheL1.Wait()

	currentGroupID.Store(2)
	svc.InvalidateAuthCacheByKey(context.Background(), "k-l1-invalidate")
