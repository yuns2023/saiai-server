...
func TestAdminService_CreateGroup_PropagatesAllowClaudeContext1MBeta(t *testing.T) {
	repo := &groupRepoStubForAdmin{}
	svc := &adminServiceImpl{groupRepo: repo}
	...
	require.NoError(t, err)
	require.NotNil(t, repo.created)
	require.True(t, repo.created.AllowClaudeContext1MBeta)
}

func TestAdminService_UpdateGroup_PropagatesAllowClaudeContext1MBeta(t *testing.T) {
	...
	_, err := svc.UpdateGroup(context.Background(), 1, &UpdateGroupInput{
		AllowClaudeContext1MBeta: &value,
	})
	require.NoError(t, err)
	require.NotNil(t, repo.updated)
