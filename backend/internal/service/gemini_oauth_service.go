...
func (s *GeminiOAuthService) GenerateAuthURL(ctx context.Context, proxyID *int64, redirectURI, projectID, oauthType, tierID string) (*GeminiAuthURLResult, error) {
	...
	// - code_assist: use the compatibility client only when its authorized env credential is configured
	// - google_one: use the compatibility client only when its authorized env credential is configured
