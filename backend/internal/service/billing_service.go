...
const (
	openAIGPTLongContextInputThreshold          = 272000
	openAIGPTLongContextInputMultiplier         = 2.0
	openAIGPTLongContextOutputMultiplier        = 1.5
	openAIGPTLongContextCacheCreationMultiplier = 2.0
	openAIGPTLongContextCacheReadMultiplier     = 2.0
	openAIGPT55PriorityMultiplier               = 2.5
)
...
// 这里仅保留 Anthropic/Gemini 等非 OpenAI 家族回退；
// OpenAI 必须依赖动态价格数据，避免近似计费。
...
func isInvalidDynamicPricing(pricing *LiteLLMModelPricing) bool {
	...
	return pricing.InputCostPerToken <= 0 || pricing.OutputCostPerToken <= 0
}

// HasModelPricing 检查模型是否有可用的定价数据
func (s *BillingService) HasModelPricing(model string) bool {
...
func (s *BillingService) CalculateCostWithServiceTier(model string, tokens UsageTokens, rateMultiplier float64, serviceTier string) (*CostBreakdown, error) {
	...
	if longContextPricing {
		...
		cacheCreationPricePerToken *= pricing.LongContextCacheCreationMultiplier
		cacheCreation5mPrice *= pricing.LongContextCacheCreationMultiplier
		cacheCreation1hPrice *= pricing.LongContextCacheCreationMultiplier
		cacheReadPricePerToken *= pricing.LongContextCacheReadMultiplier
	}
...
func (s *BillingService) applyModelSpecificPricingPolicy(model string, pricing *ModelPricing) *ModelPricing {
	...
	isLongContextModel := isOpenAILongContextGPTModel(model)
	isGPT55 := isOpenAIGPT55Model(model)
	isGPT55Pro := isOpenAIGPT55ProModel(model)
	if !isLongContextModel && !isGPT55Pro {
		return pricing
	}
	cloned := *pricing
	changed := false
	if isGPT55 {
		// LiteLLM snapshots have lagged the official 2.5x GPT-5.5 Priority price.
		// Derive these fields from the standard prices so stale 2x data cannot underbill.
		if cloned.InputPricePerToken > 0 {
			cloned.InputPricePerTokenPriority = cloned.InputPricePerToken * openAIGPT55PriorityMultiplier
			changed = true
		}
		if cloned.OutputPricePerToken > 0 {
			cloned.OutputPricePerTokenPriority = cloned.OutputPricePerToken * openAIGPT55PriorityMultiplier
			changed = true
		}
		if cloned.CacheReadPricePerToken > 0 {
			cloned.CacheReadPricePerTokenPriority = cloned.CacheReadPricePerToken * openAIGPT55PriorityMultiplier
			changed = true
		}
	}
	if isGPT55Pro && cloned.InputPricePerToken > 0 && cloned.CacheReadPricePerToken != cloned.InputPricePerToken {
		// GPT-5.5 Pro has no cached-input discount; keep this official exception
		// independent from a community pricing feed's cache-read value.
		cloned.CacheReadPricePerToken = cloned.InputPricePerToken
		changed = true
	}
	if isLongContextModel {
		if cloned.LongContextInputThreshold <= 0 {
			cloned.LongContextInputThreshold = openAIGPTLongContextInputThreshold
			changed = true
		}
		if cloned.LongContextInputMultiplier <= 0 {
			cloned.LongContextInputMultiplier = openAIGPTLongContextInputMultiplier
			changed = true
		}
		if cloned.LongContextOutputMultiplier <= 0 {
			cloned.LongContextOutputMultiplier = openAIGPTLongContextOutputMultiplier
			changed = true
		}
		if cloned.LongContextCacheCreationMultiplier <= 0 {
			cloned.LongContextCacheCreationMultiplier = openAIGPTLongContextCacheCreationMultiplier
			changed = true
		}
		if cloned.LongContextCacheReadMultiplier <= 0 {
			cloned.LongContextCacheReadMultiplier = openAIGPTLongContextCacheReadMultiplier
			changed = true
		}
	}
	if !changed {
...
func (s *BillingService) shouldApplySessionLongContextPricing(tokens UsageTokens, pricing *ModelPricing) bool {
	...
	if pricing.LongContextInputMultiplier <= 1 && pricing.LongContextOutputMultiplier <= 1 &&
		pricing.LongContextCacheCreationMultiplier <= 1 && pricing.LongContextCacheReadMultiplier <= 1 {
		return false
	}
...
func isOpenAILongContextGPTModel(model string) bool {
	normalized := normalizeOpenAIModelForBillingPolicy(model)
	switch normalized {
...
func isOpenAIGPT55ProModel(model string) bool {
	return normalizeOpenAIModelForBillingPolicy(model) == "gpt-5.5-pro"
}

func isOpenAIGPT55Model(model string) bool {
	return normalizeOpenAIModelForBillingPolicy(model) == "gpt-5.5"
}

func normalizeOpenAIModelForBillingPolicy(model string) string {
	normalized := normalizeCodexModel(strings.TrimSpace(strings.ToLower(model)))
	return openAIModelDatePattern.ReplaceAllString(normalized, "")
}
