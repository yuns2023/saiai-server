...
// 透传哲学：HTTP `Forward` 不再做账号级模型映射，本测试改为校验
// 1) 工具名称仍被 CodexToolCorrector 修正
// 2) 上游响应里的 model 字段原样回流给客户端，不再被中转层改写
func TestOpenAIGatewayService_Forward_WSv2_PassesUpstreamModelAndCorrectsToolCalls(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	require.Equal(t, "gpt-5.1", gjson.GetBytes(rec.Body.Bytes(), "model").String(), "响应模型按上游原样回流")
	require.Equal(t, "edit", gjson.GetBytes(rec.Body.Bytes(), "tool_calls.0.function.name").String(), "工具名称应被 CodexToolCorrector 修正")
}
...
func TestOpenAIGatewayService_Forward_WSv2_TurnStateAndMetadataReplayOnReconnect(t *testing.T) {
	...
	store := svc.getOpenAIWSStateStore()
	turnState, ok := store.GetSessionTurnState(0, openAIWSAccountTurnStateSessionHash(account.ID, sessionHash))
	require.True(t, ok)
