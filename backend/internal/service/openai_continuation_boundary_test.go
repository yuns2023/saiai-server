...
func TestOpenAITurnStateNeverCrossesOAuthAccounts(t *testing.T) {
	...
	store.BindSessionTurnState(17, openAIWSAccountTurnStateSessionHash(accountA.ID, sessionHash), "state-from-a", time.Hour)

	require.Equal(t, "state-from-a", svc.resolveOpenAIWSTurnStateForAccount(accountA, 17, sessionHash, "state-from-a"))
	require.Empty(t, svc.resolveOpenAIWSTurnStateForAccount(accountB, 17, sessionHash, "state-from-a"))
	store.BindSessionTurnState(17, openAIWSAccountTurnStateSessionHash(accountB.ID, sessionHash), "state-from-b", time.Hour)
	require.Equal(t, "state-from-b", svc.resolveOpenAIWSTurnStateForAccount(accountB, 17, sessionHash, "state-from-a"))
	...
	store.BindSessionTurnState(0, openAIWSAccountTurnStateSessionHash(accountB.ID, svc.GenerateSessionHash(c, nil)), "state-from-b", time.Hour)
	...
	wsState := svc.resolveOpenAIWSTurnStateForAccount(accountB, 0, svc.GenerateSessionHash(c, nil), c.GetHeader(openAIWSTurnStateHeader))
