...
func TestClaudeOAuthPendingStickyCoordinatesBeforeSuccess(t *testing.T) {
	...
	accountID, source, err := svc.GetCachedSessionAccountIDWithSource(ctx, &groupID, sessionHash)
	require.NoError(t, err)
	require.Equal(t, oauthAccount.ID, accountID)
	require.Equal(t, StickySessionBindingSourcePending, source)
	...
	require.NoError(t, err)
	require.Equal(t, oauthAccount.ID, accountID)
	require.Equal(t, StickySessionBindingSourceConfirmed, source)
