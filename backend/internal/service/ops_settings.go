...
const (
	...
	opsFullRequestBodyLoggingDefaultMaxBytes = 256 * 1024
	opsFullRequestBodyLoggingHardMaxBytes    = 2 * 1024 * 1024
	opsFullRequestBodyLoggingCacheTTL        = 5 * time.Second
)

type opsFullRequestBodyLoggingCacheEntry struct {
	cfg       *OpsFullRequestBodyLoggingSettings
	expiresAt time.Time
}
...
func defaultOpsFullRequestBodyLoggingSettings() *OpsFullRequestBodyLoggingSettings {
	return &OpsFullRequestBodyLoggingSettings{
		Enabled:   false,
		APIKeyIDs: []int64{},
		MaxBytes:  opsFullRequestBodyLoggingDefaultMaxBytes,
	}
}

func normalizeOpsFullRequestBodyLoggingSettings(cfg *OpsFullRequestBodyLoggingSettings) {
	if cfg == nil {
		return
	}
	if cfg.MaxBytes <= 0 {
		cfg.MaxBytes = opsFullRequestBodyLoggingDefaultMaxBytes
	}
	if cfg.MaxBytes > opsFullRequestBodyLoggingHardMaxBytes {
		cfg.MaxBytes = opsFullRequestBodyLoggingHardMaxBytes
	}
	cfg.ExpiresAtRFC3339 = strings.TrimSpace(cfg.ExpiresAtRFC3339)
	if len(cfg.APIKeyIDs) == 0 {
		return
	}
	seen := make(map[int64]struct{}, len(cfg.APIKeyIDs))
	out := make([]int64, 0, len(cfg.APIKeyIDs))
	for _, id := range cfg.APIKeyIDs {
		if id <= 0 {
			continue
		}
		if _, ok := seen[id]; ok {
...
func (s *OpsService) GetOpsFullRequestBodyLoggingSettings(ctx context.Context) (*OpsFullRequestBodyLoggingSettings, error) {
	defaultCfg := defaultOpsFullRequestBodyLoggingSettings()
	if s == nil || s.settingRepo == nil {
		return defaultCfg, nil
	}
	if ctx == nil {
	...
	raw, err := s.settingRepo.GetValue(ctx, SettingKeyOpsFullRequestBodyLogging)
	if err != nil {
		if errors.Is(err, ErrSettingNotFound) {
			return defaultCfg, nil
		}
	...
	cfg := defaultOpsFullRequestBodyLoggingSettings()
	if err := json.Unmarshal([]byte(raw), cfg); err != nil {
		return defaultCfg, nil
	}
	normalizeOpsFullRequestBodyLoggingSettings(cfg)
	return cfg, nil
}

func (s *OpsService) getOpsFullRequestBodyLoggingSettingsCached(ctx context.Context) (*OpsFullRequestBodyLoggingSettings, error) {
	if s == nil {
		return defaultOpsFullRequestBodyLoggingSettings(), nil
	}
	now := time.Now()
	if v := s.fullRequestBodyLoggingCache.Load(); v != nil {
		if cached, ok := v.(*opsFullRequestBodyLoggingCacheEntry); ok && cached != nil && cached.cfg != nil && now.Before(cached.expiresAt) {
			return cached.cfg, nil
		}
	}
	cfg, err := s.GetOpsFullRequestBodyLoggingSettings(ctx)
	if err != nil {
		return nil, err
	}
	s.fullRequestBodyLoggingCache.Store(&opsFullRequestBodyLoggingCacheEntry{
		cfg:       cfg,
		expiresAt: now.Add(opsFullRequestBodyLoggingCacheTTL),
	})
	return cfg, nil
}

func (s *OpsService) FullRequestBodyLoggingLimit(ctx context.Context, apiKeyID int64) (int, bool) {
	opts, ok := s.scopedRequestBodyLoggingOptions(ctx, apiKeyID, false)
	return opts.MaxBytes, ok
}

func (s *OpsService) RequestBodyCaptureOptions(ctx context.Context, apiKeyID int64) (OpsRequestBodyCaptureOptions, bool) {
	return s.scopedRequestBodyLoggingOptions(ctx, apiKeyID, false)
}

func (s *OpsService) SuccessRequestBodyCaptureLimit(ctx context.Context, apiKeyID int64) (int, bool) {
	opts, ok := s.SuccessRequestBodyCaptureOptions(ctx, apiKeyID)
	return opts.MaxBytes, ok
}

func (s *OpsService) SuccessRequestBodyCaptureOptions(ctx context.Context, apiKeyID int64) (OpsRequestBodyCaptureOptions, bool) {
	return s.scopedRequestBodyLoggingOptions(ctx, apiKeyID, true)
}

func (s *OpsService) scopedRequestBodyLoggingOptions(ctx context.Context, apiKeyID int64, successOnly bool) (OpsRequestBodyCaptureOptions, bool) {
	var zero OpsRequestBodyCaptureOptions
	if apiKeyID <= 0 {
		return zero, false
	}
	var cfg *OpsFullRequestBodyLoggingSettings
	var err error
	if successOnly {
		cfg, err = s.getOpsFullRequestBodyLoggingSettingsCached(ctx)
	} else {
		cfg, err = s.GetOpsFullRequestBodyLoggingSettings(ctx)
	}
	...
	if successOnly && !cfg.CaptureSuccess {
		return zero, false
	}
	if cfg.ExpiresAtRFC3339 != "" {
		expiresAt, err := time.Parse(time.RFC3339, cfg.ExpiresAtRFC3339)
	...
	for _, id := range cfg.APIKeyIDs {
		if id == apiKeyID {
			return OpsRequestBodyCaptureOptions{
				MaxBytes:        cfg.MaxBytes,
				CaptureSuccess:  cfg.CaptureSuccess,
...
func (s *OpsService) UpdateOpsFullRequestBodyLoggingSettings(ctx context.Context, req *OpsFullRequestBodyLoggingSettings) (*OpsFullRequestBodyLoggingSettings, error) {
	if s == nil || s.settingRepo == nil {
		return nil, errors.New("setting repository not initialized")
	}
	if ctx == nil {
	...
	if req == nil {
		req = defaultOpsFullRequestBodyLoggingSettings()
	}

	next := *req
	normalizeOpsFullRequestBodyLoggingSettings(&next)
	if next.ExpiresAtRFC3339 != "" {
		if _, err := time.Parse(time.RFC3339, next.ExpiresAtRFC3339); err != nil {
			return nil, errors.New("expires_at_rfc3339 must be RFC3339")
		}
	}
	if next.Enabled && len(next.APIKeyIDs) == 0 {
		return nil, errors.New("api_key_ids is required when request body logging is enabled")
	}
	...
	if err := s.settingRepo.Set(ctx, SettingKeyOpsFullRequestBodyLogging, string(raw)); err != nil {
		return nil, err
	}
	s.fullRequestBodyLoggingCache.Store(&opsFullRequestBodyLoggingCacheEntry{})
	return &next, nil
