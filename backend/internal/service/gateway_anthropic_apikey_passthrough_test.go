...
func TestGatewayService_AnthropicOAuth_NotAffectedByAPIKeyPassthroughToggle(t *testing.T) {
	...
	req, err := svc.buildUpstreamRequest(context.Background(), c, account, []byte(`{"model":"claude-3-7-sonnet-20250219"}`), "oauth-token", "oauth", "claude-3-7-sonnet-20250219", true, nil)
	require.NoError(t, err)
...
func TestGatewayService_AnthropicOAuth_ForwardPreservesBillingHeaderSystemBlock(t *testing.T) {
	...
	tests := []struct {
		name       string
		bodyFor    func(userID string) []byte
		wantSystem string
	}{
		{
			name: "system array",
			bodyFor: func(userID string) []byte {
				return []byte(`{"model":"claude-3-5-sonnet-latest","metadata":{"user_id":` + strconvQuote(userID) + `},"system":[{"type":"text","text":"x-anthropic-billing-header: cc_version=2.1.80.000; cc_entrypoint=cli; cch=00000; keep"}],"messages":[{"role":"user","content":[{"type":"text","text":"hello"}]}]}`)
			},
			wantSystem: "keep",
		},
		{
			name: "system string",
			bodyFor: func(userID string) []byte {
				return []byte(`{"model":"claude-3-5-sonnet-latest","metadata":{"user_id":` + strconvQuote(userID) + `},"system":"x-anthropic-billing-header: cc_version=2.1.80.000; cc_entrypoint=cli; cch=00000; keep","messages":[{"role":"user","content":[{"type":"text","text":"hello"}]}]}`)
			},
			wantSystem: "keep",
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			body := expectedOAuthBillingBodyForTest(t, tt.bodyFor(userID), req.Header.Get("User-Agent"))
			...
			require.Contains(t, system.Raw, tt.wantSystem)
		})
	}
}
...
func TestGatewayService_AnthropicAPIKeyPassthrough_StreamingTimeoutAfterClientDisconnect(t *testing.T) {
	...
	go func() {
		...
		// Keep the upstream open until the handler returns. Sleeping for a fixed
		// duration races the timeout with EOF on loaded CI runners and can exercise
		// the missing-terminal-event branch instead of the timeout branch.
		<-releaseUpstream
		_ = pw.Close()
	}()
