...
func (s *adminServiceImpl) UpdateGroup(ctx context.Context, id int64, input *UpdateGroupInput) (*Group, error) {
	...
	if input.AllowClaudeContext1MBeta != nil {
		group.AllowClaudeContext1MBeta = *input.AllowClaudeContext1MBeta
	}
...
func (s *adminServiceImpl) CreateAccount(ctx context.Context, input *CreateAccountInput) (*Account, error) {
	...
	if len(groupIDs) > 0 && !input.SkipMixedChannelCheck {
		...
		if err := s.checkClaudeOAuthModeConsistency(ctx, 0, input.Platform, input.Type, input.Extra, groupIDs); err != nil {
			return nil, err
		}
	}
...
func (s *adminServiceImpl) UpdateAccount(ctx context.Context, id int64, input *UpdateAccountInput) (*Account, error) {
	...
	applyClaudeOAuthSingleDeviceDefaults(account.Platform, account.Type, account.Extra)
	...
	if input.GroupIDs != nil {
		...
		if !input.SkipMixedChannelCheck {
			...
			targetExtra := account.Extra
			if input.Extra != nil {
				targetExtra = input.Extra
			}
			if err := s.checkClaudeOAuthModeConsistency(ctx, account.ID, account.Platform, targetType, targetExtra, *input.GroupIDs); err != nil {
				return nil, err
			}
		}
	}
...
func (s *adminServiceImpl) BulkUpdateAccounts(ctx context.Context, input *BulkUpdateAccountsInput) (*BulkUpdateAccountsResult, error) {
	...
	accountByID := make(map[int64]*Account, len(accountsForPinnedDefaults))
	for _, account := range accountsForPinnedDefaults {
		if account != nil {
	...
	if input.Extra != nil {
		allSingleDevice := true
		var mergedSingleDeviceExtra map[string]any
		targetPlatform := ""
		targetType := ""
		for _, accountID := range input.AccountIDs {
			account := accountByID[accountID]
			if account == nil {
				allSingleDevice = false
				break
			}
			targetExtra := mergedExtraMap(account.Extra, input.Extra)
			mode := strings.TrimSpace(strings.ToLower(fmt.Sprint(targetExtra["claude_oauth_mode"])))
			if account.Platform != PlatformAnthropic || (account.Type != AccountTypeOAuth && account.Type != AccountTypeSetupToken) || mode != ClaudeOAuthModeSingleDevice {
				allSingleDevice = false
				break
			}
			if mergedSingleDeviceExtra == nil {
				mergedSingleDeviceExtra = targetExtra
				targetPlatform = account.Platform
				targetType = account.Type
			}
		}
		if allSingleDevice && mergedSingleDeviceExtra != nil {
			applyClaudeOAuthSingleDeviceDefaults(targetPlatform, targetType, mergedSingleDeviceExtra)
			input.Extra = mergedSingleDeviceExtra
		}
	}
	if input.GroupIDs != nil {
		if err := s.validateGroupIDsExist(ctx, *input.GroupIDs); err != nil {
	...
	if input.GroupIDs != nil {
		for _, account := range accountsForPinnedDefaults {
			...
			if err := s.checkClaudeOAuthModeConsistency(ctx, account.ID, account.Platform, account.Type, targetExtra, *input.GroupIDs); err != nil {
				return nil, err
			}
		}
	}
...
func applyClaudeOAuthSingleDeviceDefaults(platform, accountType string, extra map[string]any) {
	if extra == nil {
		return
	}
	...
	mode := strings.TrimSpace(strings.ToLower(fmt.Sprint(extra["claude_oauth_mode"])))
	if mode != ClaudeOAuthModeSingleDevice {
		return
	}
	if _, ok := extra["claude_oauth_disable_token_refresh"]; !ok || extra["claude_oauth_disable_token_refresh"] == nil {
		extra["claude_oauth_disable_token_refresh"] = true
	}
	if _, ok := extra["claude_oauth_token_disable_before_expiry_minutes"]; !ok || extra["claude_oauth_token_disable_before_expiry_minutes"] == nil {
		extra["claude_oauth_token_disable_before_expiry_minutes"] = DefaultClaudeOAuthTokenDisableBeforeExpiryMinutes
	}
}

func mergedExtraMap(base map[string]any, overlay map[string]any) map[string]any {
...
func (s *adminServiceImpl) checkClaudeOAuthModeConsistency(ctx context.Context, currentAccountID int64, currentPlatform, currentType string, currentExtra map[string]any, groupIDs []int64) error {
	// Claude OAuth traffic modes are now allowed to coexist in one group.
	// Runtime code treats a mixed-mode group as having no group-level OAuth mode
	// and falls back to per-account handling after account selection.
	return nil
}
