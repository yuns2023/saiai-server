...
func (m *mockGatewayCacheForPlatform) RefreshSessionTTL(ctx context.Context, groupID int64, sessionHash string, ttl time.Duration) error {
	if m.refreshed == nil {
		m.refreshed = make(map[string]int)
	}
	if m.refreshTTLs == nil {
		m.refreshTTLs = make(map[string]time.Duration)
	}
	m.refreshed[sessionHash]++
	m.refreshTTLs[sessionHash] = ttl
	return nil
}
...
func TestGatewayService_SelectAccountWithLoadAwareness(t *testing.T) {
	...
	t.Run("粘性命中-不调用GetByID", func(t *testing.T) {
		...
		require.Equal(t, 1, cache.refreshed["sticky"])
		require.Equal(t, defaultStickySessionTTL, cache.refreshTTLs["sticky"])
	})
	...
	t.Run("粘性账号不在候选集-回退负载感知选择", func(t *testing.T) {
		...
		require.Equal(t, 1, repo.getByIDCalls, "应检查历史粘性账号后再回退到可用账号")
	...
	t.Run("模型路由-粘性账号等待计划", func(t *testing.T) {
		...
		require.NotNil(t, result.WaitPlan)
		require.Equal(t, int64(1), result.Account.ID)
		require.Equal(t, 1, cache.refreshed[sessionHash])
		require.Equal(t, defaultStickySessionTTL, cache.refreshTTLs[sessionHash])
	})
