...
func TestWriteFilteredHeadersReplacesExistingRequestID(t *testing.T) {
	dst := http.Header{}
	dst.Set("X-Request-ID", "local-request-uuid")

	src := http.Header{}
	src.Set("X-Request-Id", "req_01upstream")

	WriteFilteredHeaders(dst, src, nil)

	values := dst.Values("X-Request-Id")
	if len(values) != 1 {
		t.Fatalf("expected exactly one X-Request-Id value, got %v", values)
	}
	if values[0] != "req_01upstream" {
		t.Fatalf("expected upstream X-Request-Id, got %q", values[0])
	}
}
