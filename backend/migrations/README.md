...
3. **Test locally**
   ...
   make migrate-up
   ```
   If rollback is needed, write and test a separate forward migration that
   reverses the change.
...
2. **Plan reversibility without Down blocks**
   - Do not place Down SQL in the same file
   - If rollback is needed, create a new forward migration that reverses the
     previous change
