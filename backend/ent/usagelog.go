...
func (*UsageLog) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case usagelog.FieldRequestID, usagelog.FieldSessionID, usagelog.FieldModel, usagelog.FieldUpstreamModel, usagelog.FieldUserAgent, usagelog.FieldIPAddress, usagelog.FieldImageSize, usagelog.FieldMediaType:
...
func (_m *UsageLog) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case usagelog.FieldModelRateMultiplier:
			if value, ok := values[i].(*sql.NullFloat64); !ok {
				return fmt.Errorf("unexpected type %T for field model_rate_multiplier", values[i])
			} else if value.Valid {
...
func (_m *UsageLog) String() string {
	...
	builder.WriteString(fmt.Sprintf("%v", _m.ModelRateMultiplier))
	builder.WriteString(", ")
