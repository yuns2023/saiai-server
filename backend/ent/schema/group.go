...
func (Group) Fields() []ent.Field {
	return []ent.Field{
		...
		field.Bool("allow_claude_context_1m_beta").
			Default(false).
			Comment("是否保留 context-1m-2025-08-07 beta（仅 anthropic 平台使用）"),
		field.Bool("claude_oauth_request_gate_disabled").
			Default(false).
			Comment("是否关闭 Claude OAuth 请求形状保护（默认开启保护）"),
