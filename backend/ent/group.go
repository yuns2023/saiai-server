...
type Group struct {
	...
	// 是否保留 context-1m-2025-08-07 beta（仅 anthropic 平台使用）
	AllowClaudeContext1mBeta bool `json:"allow_claude_context_1m_beta,omitempty"`
	// 是否关闭 Claude OAuth 请求形状保护（默认开启保护）
	ClaudeOauthRequestGateDisabled bool `json:"claude_oauth_request_gate_disabled,omitempty"`
...
func (_m *Group) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case group.FieldAllowClaudeContext1mBeta:
			if value, ok := values[i].(*sql.NullBool); !ok {
				return fmt.Errorf("unexpected type %T for field allow_claude_context_1m_beta", values[i])
			} else if value.Valid {
				_m.AllowClaudeContext1mBeta = value.Bool
			}
		case group.FieldClaudeOauthRequestGateDisabled:
			if value, ok := values[i].(*sql.NullBool); !ok {
				return fmt.Errorf("unexpected type %T for field claude_oauth_request_gate_disabled", values[i])
			} else if value.Valid {
				_m.ClaudeOauthRequestGateDisabled = value.Bool
			}
...
func (_m *Group) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("allow_claude_context_1m_beta=")
	builder.WriteString(fmt.Sprintf("%v", _m.AllowClaudeContext1mBeta))
	builder.WriteString(", ")
	builder.WriteString("claude_oauth_request_gate_disabled=")
	builder.WriteString(fmt.Sprintf("%v", _m.ClaudeOauthRequestGateDisabled))
	builder.WriteString(", ")
