# SAIAI Server admin payment integration API
...
### 目标
本文档用于对接外部支付系统（如 `sub2apipay`）与 SAIAI Server 的 Admin API，覆盖：
- 支付成功后充值
- 用户查询
- 人工余额修正
- 前端购买页参数透传
...
### 基础地址
- HTTPS 部署：`https://<your-domain>`
- 仅限本机开发：`http://127.0.0.1:<port>`
...
当 SAIAI Server 打开 `purchase_subscription_url` 或用户侧自定义页面 iframe URL 时，会统一追加：
- `user_id`
- `token`
- `theme`（`light` / `dark`）
...
### 6) `doc_url` 配置建议
- 查看链接：`https://github.com/yuns2023/saiai-server/blob/main/docs/ADMIN_PAYMENT_INTEGRATION_API.md`
- 下载链接：`https://raw.githubusercontent.com/yuns2023/saiai-server/main/docs/ADMIN_PAYMENT_INTEGRATION_API.md`
...
### Purpose
This document describes the minimal SAIAI Server Admin API surface for external payment integrations (for example, `sub2apipay`), including:
...
- Local development only: `http://127.0.0.1:<port>`
...
When SAIAI Server opens `purchase_subscription_url` or a user-facing custom page iframe URL, it appends:
- `user_id`
- `token`
- `theme` (`light` / `dark`)
...
- View URL: `https://github.com/yuns2023/saiai-server/blob/main/docs/ADMIN_PAYMENT_INTEGRATION_API.md`
- Download URL: `https://raw.githubusercontent.com/yuns2023/saiai-server/main/docs/ADMIN_PAYMENT_INTEGRATION_API.md`
