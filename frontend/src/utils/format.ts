...
export function formatRelativeTime(date: string | Date | null | undefined): string {
  ...
  if (isNaN(diffMs)) return i18n.global.t('common.time.never')
  if (diffMs < 0) return i18n.global.t('common.time.justNow')
...
export function getEffectiveReasoningEffort(log: {
  reasoning_effort?: string | null
  reasoning_effort_effective?: string | null
}): string | null | undefined {
  const effective = (log.reasoning_effort_effective ?? '').toString().trim()
  return effective ? log.reasoning_effort_effective : log.reasoning_effort
}
