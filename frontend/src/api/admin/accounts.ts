...
export interface ClaudeCarpoolDevice {
  device_key: string
  ...
  last_arch?: string
  last_runtime?: string
  last_runtime_version?: string
  last_sdk_version?: string
}

export interface ClaudeCarpoolOverflowDevice {
  device_key: string
  original_device_id: string
  first_rejected_at: number
...
export interface ClaudeCarpoolDeviceOverview {
  ...
  recorded_items: ClaudeCarpoolDevice[]
  overflow_items: ClaudeCarpoolOverflowDevice[]
}

export interface ClaudeSharedBucket {
  bucket: number
  public_device_id: string
  last_seen_at?: number
  last_user_agent?: string
  ...
  dangerous_direct_access?: string
}

export interface ClaudePinnedBinding {
  group_id: number
  account_id: number
  account_name?: string
  account_uuid?: string
...
export interface ClaudeSingleDeviceSlot {
  slot: number
  slot_key: string
  last_seen_at?: number
  last_user_agent?: string
...
export async function listClaudeCarpoolDevices(id: number): Promise<ClaudeCarpoolDeviceOverview> {
  const { data } = await apiClient.get<ClaudeCarpoolDeviceOverview>(`/admin/accounts/${id}/claude-carpool-devices`)
  return data
}

export async function deleteClaudeCarpoolDevice(id: number, deviceKey: string): Promise<{ message: string }> {
  const { data } = await apiClient.delete<{ message: string }>(`/admin/accounts/${id}/claude-carpool-devices/${deviceKey}`)
  return data
}
...
export async function deleteClaudeSharedBucket(id: number, bucket: number): Promise<{ message: string }> {
  const { data } = await apiClient.delete<{ message: string }>(`/admin/accounts/${id}/claude-shared-buckets/${bucket}`)
  return data
}

export async function getClaudePinnedBinding(id: number): Promise<ClaudePinnedBinding | null> {
  const { data } = await apiClient.get<{ item: ClaudePinnedBinding | null }>(`/admin/accounts/${id}/claude-pinned-binding`)
  return data.item || null
}

export async function listClaudeSingleDeviceSlots(id: number): Promise<ClaudeSingleDeviceSlot[]> {
  const { data } = await apiClient.get<{ items: ClaudeSingleDeviceSlot[] }>(`/admin/accounts/${id}/claude-single-device-slots`)
  return data.items || []
}

export async function deleteClaudePinnedBinding(id: number): Promise<{ message: string }> {
  const { data } = await apiClient.delete<{ message: string }>(`/admin/accounts/${id}/claude-pinned-binding`)
  return data
}
...
export const accountsAPI = {
  ...
  getClaudePinnedBinding,
  deleteClaudePinnedBinding,
  refreshCredentials,
  getStats,
  clearError,
  getUsage,
