  ...
  >
    <div v-if="account" class="space-y-4">
      ...
      <OAuthAuthorizationFlow
        ...
        :show-setup-token-option="isAnthropic && addMethod === 'setup-token'"
...
const isManualInputMethod = computed(() => {
  // OpenAI/Gemini always use manual input (no cookie auth option)
  ...
  try {
    ...
    claudeOAuth.error.value =
      error?.message === 'Setup Token is required'
        ...
        : error?.message || error.response?.data?.detail || t('admin.accounts.oauth.authFailed')
    appStore.showError(claudeOAuth.error.value)
  } finally {
    claudeOAuth.loading.value = false
  }
