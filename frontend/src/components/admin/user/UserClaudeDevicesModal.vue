<template>
  <BaseDialog :show="show" title="Claude Code devices" width="wide" @close="emit('close')">
    <div v-if="user" class="space-y-4">
      <div class="flex items-center justify-between gap-3 rounded-xl bg-gray-50 p-4 dark:bg-dark-700">
        <div>
          <p class="font-medium text-gray-900 dark:text-white">{{ user.email }}</p>
          <p class="text-sm text-gray-500 dark:text-dark-400">User ID: {{ user.id }}</p>
        </div>
      ...
      <div class="grid grid-cols-1 gap-3 md:grid-cols-3">
        <div class="rounded-lg bg-gray-50 p-3 text-sm dark:bg-dark-700">
          <div class="text-xs text-gray-500 dark:text-gray-400">Active devices</div>
          <div class="mt-1 font-medium text-gray-900 dark:text-white">{{ activeDevices.length }}</div>
        </div>
        <div class="rounded-lg bg-gray-50 p-3 text-sm dark:bg-dark-700">
          <div class="text-xs text-gray-500 dark:text-gray-400">Revoked devices</div>
          <div class="mt-1 font-medium text-gray-900 dark:text-white">{{ revokedDevices.length }}</div>
        </div>
        <div class="rounded-lg bg-gray-50 p-3 text-sm dark:bg-dark-700">
          <div class="text-xs text-gray-500 dark:text-gray-400">Total registrations</div>
      ...
      <div v-else-if="devices.length === 0" class="py-8 text-center text-sm text-gray-500 dark:text-gray-400">
        No Claude Code devices recorded.
      </div>
          ...
          <div class="flex items-start justify-between gap-3">
            <div class="min-w-0 space-y-1">
              <div class="flex flex-wrap items-center gap-2">
                <span class="font-medium text-gray-900 dark:text-white">Device #{{ device.id }}</span>
                <span :class="['badge text-xs', device.revoked_at ? 'badge-danger' : 'badge-success']">
                  {{ device.revoked_at ? 'Revoked' : 'Active' }}
                </span>
              </div>
              <div class="flex items-center gap-2 text-sm">
                <span class="break-all font-mono text-gray-700 dark:text-gray-200">{{ device.device_id || 'Unavailable for this historical record' }}</span>
                <button v-if="device.device_id" type="button" class="btn btn-secondary btn-sm shrink-0" @click="copyDeviceID(device.device_id)">Copy</button>
              </div>
              <div class="text-xs text-gray-500 dark:text-gray-400">Group ID: {{ device.group_id }}</div>
              <div class="text-xs text-gray-500 dark:text-gray-400">First seen: {{ formatDateTime(device.first_seen_at) }}</div>
              <div class="text-xs text-gray-500 dark:text-gray-400">Last seen: {{ formatDateTime(device.last_seen_at) }}</div>
              <div v-if="device.revoked_at" class="text-xs text-gray-500 dark:text-gray-400">Revoked: {{ formatDateTime(device.revoked_at) }}</div>
            </div>
...
const load = async () => {
  ...
  try {
    ...
    appStore.showError(error?.response?.data?.detail || error?.message || 'Failed to load Claude Code devices')
  } finally {
    loading.value = false
  }
}
...
const revoke = async (device: ClaudeUserDevice) => {
  if (!props.user || !window.confirm(`Revoke Claude Code device #${device.id}?`)) return
  revokingId.value = device.id
  try {
    await adminAPI.users.revokeClaudeDevice(props.user.id, device.id)
    appStore.showSuccess('Claude Code device revoked')
    await load()
  } catch (error: any) {
    appStore.showError(error?.response?.data?.detail || error?.message || 'Failed to revoke Claude Code device')
  } finally {
...
const copyDeviceID = async (deviceID: string) => {
  try {
    ...
    appStore.showSuccess('Device ID copied')
  } catch {
    appStore.showError('Unable to copy device ID')
  }
}
</script>
