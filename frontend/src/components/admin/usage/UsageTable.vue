<template>
    ...
    <div class="overflow-auto">
      <DataTable :columns="columns" :data="data" :loading="loading">
        ...
        <template #cell-account="{ row }">
          <div class="max-w-[260px] space-y-1 text-sm">
            <div class="truncate text-gray-900 dark:text-white" :title="row.account?.name || '-'">
              {{ row.account?.name || '-' }}
            </div>
            <span
              v-if="hasFailoverEvents(row)"
              class="inline-flex cursor-help items-center rounded px-1.5 py-0.5 text-[10px] font-medium leading-tight bg-rose-100 text-rose-700 ring-1 ring-inset ring-rose-200 dark:bg-rose-500/20 dark:text-rose-300 dark:ring-rose-500/30"
              :title="formatFailoverEvents(row)"
            >
              {{ t('admin.usage.failoverBadge') }}
            </span>
          </div>
        </template>
        ...
        <template #cell-session_id="{ row }">
          <div v-if="row.session_id" class="flex max-w-[260px] items-center gap-1.5">
            <span
              class="block min-w-0 flex-1 truncate font-mono text-xs text-gray-600 dark:text-gray-400"
              :title="row.session_id"
            >
              {{ row.session_id }}
            </span>
            <button
              v-if="hasSessionAccountSwitch(row)"
              type="button"
              class="inline-flex flex-shrink-0 items-center rounded px-1.5 py-0.5 text-[10px] font-medium leading-tight bg-amber-100 text-amber-700 ring-1 ring-inset ring-amber-200 transition-colors hover:bg-amber-200 dark:bg-amber-500/20 dark:text-amber-300 dark:ring-amber-500/30"
              :title="formatSessionAccounts(row)"
              @click.stop="focusSession(row)"
            >
              {{ t('admin.usage.accountSwitchBadge', { count: row.session_account_count || 0 }) }}
            </button>
          </div>
  ...
  <Teleport to="body">
    ...
    >
      <div class="max-h-[calc(100vh-1rem)] overflow-y-auto whitespace-nowrap rounded-lg border border-gray-700 bg-gray-900 px-3 py-2.5 text-xs text-white shadow-xl dark:border-gray-600 dark:bg-gray-800">
        <div class="space-y-1.5">
          <!-- Cost Breakdown -->
          ...
          <div class="flex items-center justify-between gap-6">
            <span class="text-gray-400">{{ t('usage.groupModelMultiplier') }}</span>
...
const emit = defineEmits(['userClick', 'sessionClick'])
const { t } = useI18n()
...
const hasSessionAccountSwitch = (row: AdminUsageLog): boolean => {
  return (row.session_account_count || 0) > 1
}

const hasFailoverEvents = (row: AdminUsageLog): boolean => {
  return (row.failover_events || []).length > 0
}

const formatSessionAccounts = (row: AdminUsageLog): string => {
  const accounts = row.session_accounts || []
  if (accounts.length === 0) return t('admin.usage.accountSwitchHint')
  return accounts.map((account) => `${account.name || '-'} #${account.id}`).join(' -> ')
}

const formatFailoverEvents = (row: AdminUsageLog): string => {
  const target = row.account ? `${row.account.name || '-'} #${row.account.id}` : '-'
  const events = row.failover_events || []
  if (events.length === 0) return t('admin.usage.failoverHint')
  return events.map((event) => {
    const source = event.source_account
      ? `${event.source_account.name || '-'} #${event.source_account.id}`
      : '-'
    const status = event.status_code ? `HTTP ${event.status_code}` : ''
    const sourceText = `${source} -> ${target}`
    const message = [event.message, status].filter(Boolean).join(' / ')
    const detail = event.detail || event.error_source || event.error_type || ''
    return [sourceText, message, detail].filter(Boolean).join('\n')
  }).join('\n\n')
}

const focusSession = (row: AdminUsageLog) => {
  if (!row.session_id) return
  emit('sessionClick', row.session_id)
}
