    ...
    <div class="flex flex-wrap items-end justify-between gap-4">
      ...
      <div class="flex flex-1 flex-wrap items-end gap-4">
        ...
        <div class="relative w-full sm:w-auto sm:min-w-[260px]">
          <label class="input-label">{{ t('usage.sessionId') }}</label>
          <input
          ...
          <button
            ...
            class="absolute right-2 top-9 text-gray-400"
            aria-label="Clear session ID filter"
          >
            ✕
          </button>
        </div>
        ...
        <!-- Session account-switch filter -->
        <div v-if="showSessionDiagnostics" class="w-full sm:w-auto sm:min-w-[190px]">
          <label class="input-label">{{ t('admin.usage.sessionDiagnostics') }}</label>
          <label class="flex h-10 cursor-pointer items-center gap-2 rounded-md border border-gray-300 bg-white px-3 text-sm text-gray-700 transition-colors hover:bg-gray-50 dark:border-dark-600 dark:bg-dark-700 dark:text-gray-200 dark:hover:bg-dark-600">
            <input
...
const clearSessionId = () => {
  filters.value.session_id = undefined
  emitChange()
}
