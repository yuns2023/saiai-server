  ...
  >
    ...
    <div v-else class="space-y-5">
      <OAuthAuthorizationFlow
        ...
        :show-credentials-json-option="form.platform === 'anthropic' && addMethod === 'oauth' && claudeOAuthMode === 'single_device'"
        :show-setup-token-option="form.platform === 'anthropic' && addMethod === 'setup-token'"
        :platform="form.platform"
        :show-project-id="geminiOAuthType === 'code_assist'"
        @generate-url="handleGenerateUrl"
        @cookie-auth="handleCookieAuth"
        @validate-refresh-token="handleValidateRefreshToken"
        @import-claude-credentials-json="handleImportClaudeCredentialsJson"
        @import-claude-setup-token="handleImportClaudeSetupToken"
      />
...
// OpenAI 走透传，账号级模型白名单/映射功能已下线，OpenAI 平台始终视为禁用模型限制。
const isOpenAIModelRestrictionDisabled = computed(() => form.platform === 'openai')
...
const buildGroupCompatibilityCheckPayload = () => {
  ...
  if (form.platform === 'anthropic' && (form.type === 'oauth' || form.type === 'setup-token')) {
...
interface ClaudeCredentialsJsonPayload {
  claudeAiOauth?: {
    accessToken?: string
    refreshToken?: string
    expiresAt?: number
...
const parseClaudeCredentialsJson = (raw: string) => {
  let parsed: ClaudeCredentialsJsonPayload
  try {
    parsed = JSON.parse(raw) as ClaudeCredentialsJsonPayload
  } catch {
    throw new Error('Invalid .credentials.json: JSON parse failed')
  }

  const oauthPayload = parsed?.claudeAiOauth
  if (!oauthPayload || typeof oauthPayload !== 'object') {
    throw new Error('Invalid .credentials.json: missing claudeAiOauth')
  }

  const accessToken = typeof oauthPayload.accessToken === 'string' ? oauthPayload.accessToken.trim() : ''
  const refreshToken = typeof oauthPayload.refreshToken === 'string' ? oauthPayload.refreshToken.trim() : ''
  const expiresAtRaw = Number(oauthPayload.expiresAt)
  const expiresAt = expiresAtRaw > 1_000_000_000_000 ? Math.floor(expiresAtRaw / 1000) : Math.floor(expiresAtRaw)

  if (!accessToken) {
    throw new Error('Invalid .credentials.json: missing claudeAiOauth.accessToken')
  }
  if (!refreshToken) {
    throw new Error('Invalid .credentials.json: missing claudeAiOauth.refreshToken')
  }
  if (!Number.isFinite(expiresAt) || expiresAt <= 0) {
    throw new Error('Invalid .credentials.json: invalid claudeAiOauth.expiresAt')
  }
  ...
  const scopes = Array.isArray(oauthPayload.scopes)
    ? oauthPayload.scopes.filter((item): item is string => typeof item === 'string').map((item) => item.trim()).filter(Boolean)
    : []
  if (scopes.length > 0) {
  ...
  if (typeof oauthPayload.subscriptionType === 'string' && oauthPayload.subscriptionType.trim()) {
    baseExtra.subscription_type = oauthPayload.subscriptionType.trim()
  }
  if (typeof oauthPayload.rateLimitTier === 'string' && oauthPayload.rateLimitTier.trim()) {
    baseExtra.rate_limit_tier = oauthPayload.rateLimitTier.trim()
  }

  return { credentials, baseExtra }
}

const buildAnthropicOAuthExtra = (baseExtra: Record<string, unknown>) => {
  ...
  extra.claude_oauth_mode = claudeOAuthMode.value
  ...
  delete extra.claude_oauth_single_device_admission_enabled
  delete extra.claude_oauth_single_device_admission_auto_expand_enabled
  if (claudeOAuthMode.value === 'shared') {
    ...
    extra.claude_oauth_single_device_admission_enabled = claudeOAuthSingleDeviceAdmissionEnabled.value
    if (claudeOAuthSingleDeviceAdmissionEnabled.value) {
      extra.claude_oauth_single_device_admission_limit = Math.min(32, Math.max(1, claudeOAuthSingleDeviceAdmissionLimit.value || 5))
      extra.claude_oauth_single_device_admission_target = Math.min(32, Math.max(1, claudeOAuthSingleDeviceAdmissionTarget.value || 16))
      extra.claude_oauth_single_device_admission_auto_expand_enabled = claudeOAuthSingleDeviceAdmissionAutoExpand.value
    }
    ...
    extra.claude_oauth_fixed_headers_enabled = claudeOAuthFixedHeadersEnabled.value
    if (claudeOAuthFixedHeadersText.value.trim()) {
      if (claudeOAuthFixedHeadersEnabled.value) {
        ...
        extra.claude_oauth_fixed_headers_saved_text = claudeOAuthFixedHeadersText.value.trim()
      }
    }
  }
...
const handleAnthropicExchange = async (authCode: string) => {
  ...
  try {
    ...
    try {
      ...
      oauth.error.value =
        error.response?.data?.detail || t('admin.accounts.oauth.failedToExchangeCode')
      appStore.showError(oauth.error.value)
      return
    }

    const baseExtra = oauth.buildExtraInfo(tokenInfo) || {}
    const extra = buildAnthropicOAuthExtra(baseExtra)
    ...
    try {
      ...
      oauth.error.value = detail
        ? `${t('admin.accounts.failedToCreate')}: ${detail}`
...
const handleImportClaudeSetupToken = async (raw: string) => {
  oauth.loading.value = true
  oauth.error.value = ''

  try {
    const credentials = buildClaudeSetupTokenCredentials(raw)
    applyInterceptWarmup(credentials, interceptWarmupRequests.value, 'create')
    const extra = buildAnthropicOAuthExtra({})
    await createAccountAndFinish('anthropic', 'setup-token', credentials, extra)
  } catch (error: any) {
    oauth.error.value =
      error?.message === 'Setup Token is required'
        ? t('admin.accounts.oauth.setupTokenDirectRequired')
        : error?.message || error?.response?.data?.detail || t('admin.accounts.failedToCreate')
    appStore.showError(oauth.error.value)
  } finally {
    oauth.loading.value = false
  }
...
const handleImportClaudeCredentialsJson = async (raw: string) => {
  oauth.loading.value = true
  oauth.error.value = ''

  try {
    const parsed = parseClaudeCredentialsJson(raw)
    const extra = buildAnthropicOAuthExtra(parsed.baseExtra)
    const credentials = { ...parsed.credentials }
    applyInterceptWarmup(credentials, interceptWarmupRequests.value, 'create')
    await createAccountAndFinish('anthropic', 'oauth', credentials, extra)
  } catch (error: any) {
    oauth.error.value = error?.message || error?.response?.data?.detail || t('admin.accounts.failedToCreate')
    appStore.showError(oauth.error.value)
  } finally {
    oauth.loading.value = false
  }
