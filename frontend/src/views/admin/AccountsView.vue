        ...
        >
          ...
          <template #cell-actions="{ row }">
            <div class="flex items-center gap-1">
              <button v-if="!isRetiredPlatform(row.platform)" @click="handleEdit(row)" class="flex flex-col items-center gap-0.5 rounded-lg p-1.5 text-gray-500 transition-colors hover:bg-gray-100 hover:text-primary-600 dark:hover:bg-dark-700 dark:hover:text-primary-400">
...
const DEFAULT_HIDDEN_COLUMNS = ['today_stats', 'proxy', 'notes', 'priority', 'rate_multiplier', 'payg_discount_multiplier']
...
const allColumns = computed(() => {
  ...
  c.push(
    ...
    { key: 'payg_discount_multiplier', label: t('admin.accounts.columns.paygDiscountMultiplier'), sortable: false },
...
// Columns that can be toggled (exclude fixed identity/action columns)
...
const copyAccountId = async (id: number) => {
  await copyToClipboard(String(id), 'Account ID copied')
}
