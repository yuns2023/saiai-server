<template>
  <AppLayout>
    <div class="space-y-6">
      ...
      <UsageTable :data="usageLogs" :loading="loading" :columns="visibleColumns" @userClick="handleUserClick" @sessionClick="handleSessionClick" />
...
import { formatReasoningEffort, getEffectiveReasoningEffort } from '@/utils/format'
...
const exportToCsv = async () => {
  ...
  try {
    ...
    const headers = [
      ...
      t('usage.rate'), t('usage.groupModelMultiplier'), t('usage.paygUserDiscount'), t('usage.legacyPaygAccountDiscount'), t('usage.accountMultiplier'), t('usage.original'), t('usage.userBilled'), t('usage.accountBilled'),
      t('usage.firstToken'), t('usage.duration'),
      t('admin.usage.requestId'), t('usage.userAgent'), t('admin.usage.ipAddress')
    ]
    const csvParts: string[] = ['\uFEFF', headers.map(escapeCsvCell).join(','), '\r\n']
    while (true) {
      const requestType = filters.value.request_type
      const legacyStream = requestType ? requestTypeToLegacyStream(requestType) : filters.value.stream
      ...
      const rows = (res.items || []).map((log: AdminUsageLog) => [
        ...
        log.upstream_model || '', formatReasoningEffort(getEffectiveReasoningEffort(log)), log.group?.name || '',
        log.inbound_endpoint || '', log.upstream_endpoint || '', log.session_id || '', getRequestTypeLabel(log),
        log.input_tokens, log.output_tokens, log.cache_read_tokens, log.cache_creation_tokens,
        log.input_cost?.toFixed(6) || '0.000000', log.output_cost?.toFixed(6) || '0.000000',
        log.cache_read_cost?.toFixed(6) || '0.000000', log.cache_creation_cost?.toFixed(6) || '0.000000',
        log.rate_multiplier?.toFixed(2) || '1.00', (log.model_rate_multiplier ?? 1).toFixed(4), (log.user_payg_discount_multiplier ?? 1).toFixed(4), (log.account_payg_discount_multiplier ?? 1).toFixed(4), (log.account_rate_multiplier ?? 1).toFixed(2),
        log.total_cost?.toFixed(6) || '0.000000', log.actual_cost?.toFixed(6) || '0.000000',
        (log.total_cost * (log.account_rate_multiplier ?? 1)).toFixed(6), log.first_token_ms ?? '', log.duration_ms,
        log.request_id || '', log.user_agent || '', log.ip_address || ''
      ])
      if (rows.length) {
        csvParts.push(rows.map((row) => row.map(escapeCsvCell).join(',')).join('\r\n'), '\r\n')
      }
