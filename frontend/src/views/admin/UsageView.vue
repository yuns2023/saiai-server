...
const exportToCsv = async () => {
  ...
  try {
    ...
    const headers = [
      ...
      t('usage.rate'), t('usage.groupModelMultiplier'), t('usage.paygAccountDiscount'), t('usage.accountMultiplier'), t('usage.original'), t('usage.userBilled'), t('usage.accountBilled'),
    ...
    while (true) {
      ...
      const rows = (res.items || []).map((log: AdminUsageLog) => [
        ...
        log.rate_multiplier?.toFixed(2) || '1.00', (log.model_rate_multiplier ?? 1).toFixed(4), (log.account_payg_discount_multiplier ?? 1).toFixed(4), (log.account_rate_multiplier ?? 1).toFixed(2),
