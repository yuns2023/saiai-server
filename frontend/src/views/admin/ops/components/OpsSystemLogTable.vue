...
const captureDetailRow = ref<OpsSystemLog | null>(null)
const copiedCaptureSection = ref('')
...
const getExtraValue = (row: OpsSystemLog | null, key: string) => {
  if (!row || !row.extra) return undefined
  return row.extra[key]
}

const isBodyCaptureLog = (row: OpsSystemLog) => {
  const extra = row.extra || {}
  return row.component === 'audit.request_body_capture' || extra.request_body != null || extra.upstream_request_body != null
}

const openCaptureDetail = (row: OpsSystemLog) => {
  captureDetailRow.value = row
}

const closeCaptureDetail = () => {
  captureDetailRow.value = null
  copiedCaptureSection.value = ''
}

const prettyPrintJsonPreserveOrder = (value: string) => {
  ...
  const appendIndent = () => {
    output += indentUnit.repeat(Math.max(indent, 0))
  }
  ...
  for (let i = 0; i < input.length; i += 1) {
    ...
    switch (ch) {
      ...
      case '[': {
        output += ch
        const close = ch === '{' ? '}' : ']'
        if (nextNonWhitespace(i + 1) !== close) {
          indent += 1
          appendNewline()
        }
        break
      ...
      case ']': {
        const open = ch === '}' ? '{' : '['
        if (previousNonWhitespace(i - 1) !== open) {
...
const formatCaptureBody = (value: any) => {
  if (value == null || value === '') return '-'
  if (typeof value === 'string') {
    const trimmed = value.trim()
    if (!trimmed) return '-'
    return prettyPrintJsonPreserveOrder(trimmed) || trimmed
  }
  return formatCaptureValue(value)
}

const formatCaptureValue = (value: any) => {
  if (value == null || value === '') return '-'
  if (typeof value === 'string') {
    const trimmed = value.trim()
    if (!trimmed) return '-'
...
const formatCaptureHeaders = (value: any) => {
  if (Array.isArray(value)) {
    const lines = value.map((item, idx) => {
      const index = Number(item?.index || idx + 1)
      const name = String(item?.name || '').trim()
      const headerValue = String(item?.value ?? '')
      if (!name) return ''
      return `${index}. ${name}: ${headerValue}`
    }).filter(Boolean)
...
const getCaptureSectionText = (section: string) => {
  const row = captureDetailRow.value
  if (!row) return ''
  switch (section) {
    case 'inbound_headers':
      return formatCaptureHeaders(getExtraValue(row, 'inbound_request_headers'))
    case 'inbound_body':
      return formatCaptureBody(getExtraValue(row, 'request_body'))
    case 'outbound_headers':
      return formatCaptureHeaders(getExtraValue(row, 'upstream_request_headers'))
    case 'outbound_body':
      return formatCaptureBody(getExtraValue(row, 'upstream_request_body'))
    default:
      return ''
  }
}

const copyCaptureSection = async (section: string) => {
  const text = getCaptureSectionText(section)
  if (!text || text === '-') {
    appStore.showError('没有可复制的内容')
    return
  }
  try {
    const success = await copyToClipboard(text, '已复制')
    if (!success) return
    copiedCaptureSection.value = section
    setTimeout(() => {
      if (copiedCaptureSection.value === section) copiedCaptureSection.value = ''
    }, 900)
  } catch (err) {
    console.error('[OpsSystemLogTable] Failed to copy capture section', err)
    appStore.showError('复制失败')
  }
}

const captureMetaRows = computed(() => {
  const row = captureDetailRow.value
  if (!row) return []
  const extra = row.extra || {}
  return [
    ['时间', formatTime(row.created_at)],
    ['request_id', row.request_id || getExtraString(extra, 'request_id') || '-'],
    ['API Key', [getExtraString(extra, 'api_key_name'), getExtraString(extra, 'api_key_id')].filter(Boolean).join(' / ') || '-'],
    ['账号', [getExtraString(extra, 'account_name'), getExtraString(extra, 'account_id')].filter(Boolean).join(' / ') || '-'],
    ['平台 / 模型', [row.platform || getExtraString(extra, 'platform'), row.model || getExtraString(extra, 'model')].filter(Boolean).join(' / ') || '-'],
    ['会话', getExtraString(extra, 'session_id') || '-'],
    ['入口 / 上游', [getExtraString(extra, 'inbound_endpoint'), getExtraString(extra, 'upstream_endpoint')].filter(Boolean).join(' -> ') || '-'],
    ['上游 URL', [getExtraString(extra, 'upstream_request_method'), getExtraString(extra, 'upstream_request_url')].filter(Boolean).join(' ') || '-'],
    ['Body 字节', `${getExtraString(extra, 'request_body_bytes') || '-'} / ${getExtraString(extra, 'upstream_request_body_bytes') || '-'}`],
    ['Body 裁剪', `${getExtraString(extra, 'request_body_truncated') || 'false'} / ${getExtraString(extra, 'upstream_request_body_truncated') || 'false'}`]
  ]
})
...
<template>
            ...
            <tr v-for="row in logs" :key="row.id" class="align-top">
              ...
              <td class="px-3 py-2 text-xs text-gray-700 dark:text-gray-300 whitespace-normal break-all">
                <div class="space-y-2">
                  ...
                  <button
                    v-if="isBodyCaptureLog(row)"
                    type="button"
                    class="btn btn-secondary btn-xs"
                    @click="openCaptureDetail(row)"
                  >
                    查看请求快照
                  </button>
                </div>
  ...
  <BaseDialog :show="!!captureDetailRow" title="请求快照" width="extra-wide" @close="closeCaptureDetail">
    <div v-if="captureDetailRow" class="space-y-4">
      <div class="grid grid-cols-1 gap-2 text-xs md:grid-cols-2">
        <div
          v-for="[label, value] in captureMetaRows"
          :key="label"
          class="rounded-md border border-gray-200 p-2 dark:border-dark-700"
        >
          <div class="font-semibold text-gray-500 dark:text-gray-400">{{ label }}</div>
          <div class="mt-1 break-all text-gray-900 dark:text-gray-100">{{ value }}</div>
        </div>
      </div>

      <div>
        <div class="mb-2 flex items-center justify-between gap-2">
          <div class="text-xs font-semibold text-gray-700 dark:text-gray-200">入站 Header（有序，已脱敏）</div>
          <button type="button" class="btn btn-secondary btn-xs" @click="copyCaptureSection('inbound_headers')">
      ...
      <div>
        <div class="mb-2 flex items-center justify-between gap-2">
          <div class="text-xs font-semibold text-gray-700 dark:text-gray-200">入站 Body（已格式化，保留字段顺序）</div>
          <button type="button" class="btn btn-secondary btn-xs" @click="copyCaptureSection('inbound_body')">
      ...
      <div>
        <div class="mb-2 flex items-center justify-between gap-2">
          <div class="text-xs font-semibold text-gray-700 dark:text-gray-200">出站 Header（有序，已脱敏）</div>
          <button type="button" class="btn btn-secondary btn-xs" @click="copyCaptureSection('outbound_headers')">
      ...
      <div>
        <div class="mb-2 flex items-center justify-between gap-2">
          <div class="text-xs font-semibold text-gray-700 dark:text-gray-200">出站 Body（已格式化，保留字段顺序）</div>
          <button type="button" class="btn btn-secondary btn-xs" @click="copyCaptureSection('outbound_body')">
