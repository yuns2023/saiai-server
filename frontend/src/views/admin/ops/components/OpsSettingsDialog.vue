...
const bodyCaptureSettings = ref<OpsFullRequestBodyLoggingSettings | null>(null)
const bodyCaptureApiKeyIDsText = ref('')
// 指标阈值配置
...
async function loadAllSettings() {
  loading.value = true
  try {
    const [runtime, email, advanced, thresholds, bodyCapture] = await Promise.all([
      opsAPI.getAlertRuntimeSettings(),
      opsAPI.getEmailNotificationConfig(),
      opsAPI.getAdvancedSettings(),
      opsAPI.getMetricThresholds(),
      opsAPI.getFullRequestBodyLoggingSettings()
    ])
    runtimeSettings.value = runtime
    emailConfig.value = email
    advancedSettings.value = advanced
    bodyCaptureSettings.value = {
      ...bodyCapture,
      capture_upstream: bodyCapture.capture_upstream ?? false
    }
    bodyCaptureApiKeyIDsText.value = (bodyCapture.api_key_ids || []).join(', ')
...
function parseBodyCaptureApiKeyIDs(): number[] {
  const seen = new Set<number>()
  const ids: number[] = []
  for (const part of bodyCaptureApiKeyIDsText.value.split(/[,\s]+/)) {
...
const validation = computed(() => {
  ...
  if (bodyCaptureSettings.value) {
    const ids = parseBodyCaptureApiKeyIDs()
    if (bodyCaptureSettings.value.enabled && ids.length === 0) {
      errors.push(t('admin.ops.settings.validation.bodyCaptureApiKeysRequired'))
    }
    if (bodyCaptureSettings.value.max_bytes < 1024 || bodyCaptureSettings.value.max_bytes > 2097152) {
      errors.push(t('admin.ops.settings.validation.bodyCaptureMaxBytesRange'))
    }
  }
...
async function saveAllSettings() {
  ...
  try {
    ...
    if (bodyCaptureSettings.value) {
      bodyCaptureSettings.value.api_key_ids = parseBodyCaptureApiKeyIDs()
      if (!bodyCaptureSettings.value.enabled) {
        bodyCaptureSettings.value.capture_success = false
        bodyCaptureSettings.value.capture_upstream = false
      }
      if (!bodyCaptureSettings.value.capture_success) {
        bodyCaptureSettings.value.capture_upstream = false
      }
    }
    await Promise.all([
...
<template>
  <BaseDialog :show="show" :title="t('admin.ops.settings.title')" width="extra-wide" @close="emit('close')">
    ...
    <div v-else-if="runtimeSettings && emailConfig && advancedSettings && bodyCaptureSettings" class="space-y-6">
      <!-- 验证错误 -->
      ...
      <details class="rounded-2xl bg-gray-50 dark:bg-dark-700/50">
        ...
        <div class="space-y-4 px-4 pb-4">
          ...
          <div class="space-y-3">
            <h5 class="text-xs font-semibold text-gray-700 dark:text-gray-300">{{ t('admin.ops.settings.requestBodyCapture') }}</h5>

            <div class="flex items-center justify-between">
              <div>
                <label class="text-sm font-medium text-gray-700 dark:text-gray-300">{{ t('admin.ops.settings.enableRequestBodyCapture') }}</label>
                <p class="mt-1 text-xs text-gray-500">{{ t('admin.ops.settings.enableRequestBodyCaptureHint') }}</p>
              </div>
              <Toggle v-model="bodyCaptureSettings.enabled" />
            </div>

            <div v-if="bodyCaptureSettings.enabled" class="grid grid-cols-1 gap-4 md:grid-cols-2">
              <div class="md:col-span-2">
                <label class="input-label">{{ t('admin.ops.settings.requestBodyCaptureApiKeyIds') }}</label>
                <textarea
                  v-model="bodyCaptureApiKeyIDsText"
                  rows="2"
                  class="input resize-none"
                  :placeholder="t('admin.ops.settings.requestBodyCaptureApiKeyIdsPlaceholder')"
                />
                <p class="mt-1 text-xs text-gray-500">{{ t('admin.ops.settings.requestBodyCaptureApiKeyIdsHint') }}</p>
              </div>

              <div class="flex items-center justify-between rounded-md border border-gray-200 p-3 dark:border-gray-700">
                <div>
                  <label class="text-sm font-medium text-gray-700 dark:text-gray-300">{{ t('admin.ops.settings.captureSuccessfulRequests') }}</label>
                  <p class="mt-1 text-xs text-gray-500">{{ t('admin.ops.settings.captureSuccessfulRequestsHint') }}</p>
                </div>
                <Toggle v-model="bodyCaptureSettings.capture_success" />
              </div>

              <div class="flex items-center justify-between rounded-md border border-gray-200 p-3 dark:border-gray-700">
                <div>
                  <label class="text-sm font-medium text-gray-700 dark:text-gray-300">{{ t('admin.ops.settings.captureUpstreamRequests') }}</label>
                  <p class="mt-1 text-xs text-gray-500">{{ t('admin.ops.settings.captureUpstreamRequestsHint') }}</p>
                </div>
                <Toggle v-model="bodyCaptureSettings.capture_upstream" :disabled="!bodyCaptureSettings.capture_success" />
              </div>

              <div>
                <label class="input-label">{{ t('admin.ops.settings.requestBodyCaptureMaxBytes') }}</label>
                <input
                  v-model.number="bodyCaptureSettings.max_bytes"
                  type="number"
                  min="1024"
                  max="2097152"
                  class="input"
              ...
              <div class="md:col-span-2">
                <label class="input-label">{{ t('admin.ops.settings.requestBodyCaptureExpiresAt') }}</label>
                <input
                  v-model="bodyCaptureSettings.expires_at_rfc3339"
                  type="text"
                  class="input"
                  placeholder="2026-05-22T12:00:00Z"
                />
                <p class="mt-1 text-xs text-gray-500">{{ t('admin.ops.settings.requestBodyCaptureExpiresAtHint') }}</p>
              </div>
            </div>
          </div>
