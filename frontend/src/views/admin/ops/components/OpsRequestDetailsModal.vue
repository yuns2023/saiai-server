...
function getErrorOriginLabel(row: OpsRequestDetail): string {
  if (row.kind !== 'error') return '-'
  const owner = String(row.error_owner || '').toLowerCase()
  ...
  if (owner === 'platform' || source === 'gateway') {
    return t('admin.ops.requestDetails.origin.platform')
  }
  if (owner === 'client' || source === 'client_request') {
    return t('admin.ops.requestDetails.origin.client')
  }
  if (phase === 'upstream' || phase === 'network') {
    return t('admin.ops.requestDetails.origin.provider')
  }
  if (phase === 'routing' || phase === 'internal') {
    return t('admin.ops.requestDetails.origin.platform')
  }
...
function getErrorOriginDetail(row: OpsRequestDetail): string {
  if (row.kind !== 'error') return ''
  const source = String(row.error_source || '').toLowerCase()
  const phase = String(row.phase || '').toLowerCase()
  ...
  if (source === 'client_request') {
    return t('admin.ops.requestDetails.originDetail.client_request')
  }

  if (phase) {
    const phaseLabel = t(`admin.ops.errorDetails.phase.${phase}`)
    return t('admin.ops.requestDetails.originDetail.phaseDerived', { phase: phaseLabel })
  }
  return ''
}

function getAPIKeyLabel(row: OpsRequestDetail): string {
  const name = String(row.api_key_name || '').trim()
  if (name) return name
  if (row.api_key_id != null) return `#${row.api_key_id}`
  return '-'
}

function getAPIKeyTitle(row: OpsRequestDetail): string {
  const name = String(row.api_key_name || '').trim()
  if (name && row.api_key_id != null) return `${name} (#${row.api_key_id})`
  if (name) return name
  if (row.api_key_id != null) return `#${row.api_key_id}`
  return ''
}
            ...
            <div class="min-h-0 flex-1 overflow-auto">
              ...
              <tbody class="divide-y divide-gray-200 bg-white dark:divide-dark-700 dark:bg-dark-800">
                <tr v-for="(row, idx) in items" :key="idx" class="hover:bg-gray-50 dark:hover:bg-dark-700/50">
                  ...
                  <td class="max-w-[180px] truncate px-4 py-3 text-xs font-medium text-gray-700 dark:text-gray-200" :title="getAPIKeyTitle(row)">
                    {{ getAPIKeyLabel(row) }}
                  </td>
                  <td class="whitespace-nowrap px-4 py-3 text-xs text-gray-600 dark:text-gray-300">
                  ...
                  <td class="px-4 py-3 text-xs text-gray-600 dark:text-gray-300">
                    <div v-if="row.kind === 'error'" class="flex flex-col">
                      <span class="font-medium text-gray-700 dark:text-gray-200">
                        {{ getErrorOriginLabel(row) }}
                      </span>
                      <span
                        v-if="getErrorOriginDetail(row)"
