    ...
    <div v-else class="space-y-6 p-6">
      ...
      <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <div class="rounded-xl bg-gray-50 p-4 dark:bg-dark-900">
          ...
          <div class="mt-1 truncate text-sm font-medium text-gray-900 dark:text-white" :title="apiKeyTitle">
            {{ apiKeyLabel }}
          </div>
        </div>
      ...
      <div v-if="showUpstreamList" class="rounded-xl bg-gray-50 p-6 dark:bg-dark-900">
        ...
        <div v-if="!correlatedUpstreamLoading && !embeddedUpstreamEvents.length && !correlatedUpstreamErrors.length" class="mt-3 text-sm text-gray-500 dark:text-gray-400">
          {{ t('common.noData') }}
        </div>

        <div v-else class="mt-4 space-y-3">
          <div
            v-for="(ev, idx) in embeddedUpstreamEvents"
            :key="ev.key"
          ...
          >
            <div class="flex flex-wrap items-center justify-between gap-2">
              ...
              <button
                ...
                :disabled="!ev.detail"
                :title="ev.detail ? '' : t('common.noData')"
                @click="toggleEmbeddedDetail(ev.key)"
              >
                <Icon
                  :name="expandedEmbeddedDetailKeys.has(ev.key) ? 'chevronDown' : 'chevronRight'"
                  size="xs"
                  :stroke-width="2"
                />
                <span>
                  {{
                    expandedEmbeddedDetailKeys.has(ev.key)
            ...
            <pre
              v-if="expandedEmbeddedDetailKeys.has(ev.key)"
              class="mt-3 max-h-[320px] overflow-auto rounded-xl border border-gray-200 bg-gray-50 p-3 text-xs text-gray-800 dark:border-dark-700 dark:bg-dark-900 dark:text-gray-100"
          ...
          >
            <div class="flex flex-wrap items-center justify-between gap-2">
              <div class="text-xs font-black text-gray-900 dark:text-white">
                #{{ embeddedUpstreamEvents.length + idx + 1 }}
...
const apiKeyLabel = computed(() => {
  const name = String(detail.value?.api_key_name || '').trim()
  if (name) return name
  if (detail.value?.api_key_id != null) return `#${detail.value.api_key_id}`
  return '—'
})

const apiKeyTitle = computed(() => {
  const name = String(detail.value?.api_key_name || '').trim()
  if (name && detail.value?.api_key_id != null) return `${name} (#${detail.value.api_key_id})`
  if (name) return name
  if (detail.value?.api_key_id != null) return `#${detail.value.api_key_id}`
  return ''
})
...
function toggleEmbeddedDetail(key: string) {
  const next = new Set(expandedEmbeddedDetailKeys.value)
  if (next.has(key)) next.delete(key)
  else next.add(key)
  expandedEmbeddedDetailKeys.value = next
}
...
watch(
  ...
  ([show, id]) => {
    ...
    if (typeof id === 'number' && id > 0) {
      ...
      expandedEmbeddedDetailKeys.value = new Set()
      fetchDetail(id)
