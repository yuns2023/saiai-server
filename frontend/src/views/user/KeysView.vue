        ...
        <DataTable :columns="columns" :data="apiKeys" :loading="loading">
          <template #cell-id="{ value }">
            <div class="flex items-center gap-2">
              ...
              <button
                @click="copyApiKeyId(value)"
                class="rounded-lg p-1 transition-colors hover:bg-gray-100 dark:hover:bg-dark-700"
                :class="
                  copiedApiKeyId === value
                    ? 'text-green-500'
                    : 'text-gray-400 hover:text-gray-600 dark:hover:text-gray-300'
                "
                :title="copiedApiKeyId === value ? t('keys.copied') : t('keys.copyToClipboard')"
              >
                <Icon
                  v-if="copiedApiKeyId === value"
                  name="check"
                  size="sm"
                  :stroke-width="2"
                />
...
const copyApiKeyId = async (keyId: number) => {
  ...
  if (success) {
    copiedApiKeyId.value = keyId
    setTimeout(() => {
      copiedApiKeyId.value = null
    }, 800)
  }
}
