...
API Key、转发支持的供应商协议、执行账号调度与用量控制。配套的 V2 上线 PR 将加入
SAIAI V2 客户端使用的 schema-2 bootstrap 协议。
...
- 经过审查的 SAIAI V2 bootstrap 候选实现，不产生模型调用费用
...
Schema-2 实现由首个公开 V2 上线 PR 引入。在该变更合并之前，公开 `main` 分支不承诺
schema-2 兼容性。
